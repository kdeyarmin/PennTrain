...
| Idea | Verdict | Why |
|------|---------|-----|
| eMAR / resident clinical records | ~~**No**~~ → **Building (2026-07)** | _Original rationale (retained for history):_ ECP's 850+ pharmacy-integration network is a multi-year moat; segment saturated; patient-safety liability the schema deliberately avoids. **Superseded by product decision:** EHR/clinical capability is now being built as a hybrid model — native clinical capture (vitals, care plans, assessments, notes) plus FHIR R4 ingestion of externally-owned data (medications, allergies, diagnoses, orders). Patient-safety/liability is addressed via RLS, HIPAA access auditing, consent tracking, and append-only clinical evidence. See docs/HIPAA_CLINICAL_DATA.md. |
| Qualification-gated shift scheduling | **Built**, including the qualification gate | The employees-single-facility blocker this line cited is resolved: `employee_facility_assignments` is the facility join table for the roster (mirrors the existing profile-level `facility_assignments`), additive alongside `employees.facility_id` as the home/primary facility. On top of that, a shift-scheduling module shipped -- `facility_units` (wings), `shift_definitions` (typical shift templates), `employee_schedule_preferences` (each employee's typical shift/unit pattern), `schedules` (draft/published periods), and `shift_assignments`, with a `generate_schedule_assignments` auto-fill RPC that prioritizes each employee's typical pattern so a manager isn't arranging every cell by hand. `/app/schedule` (org_admin/facility_manager) and `/me/schedule` (employee, published shifts only) in the frontend. Both halves this line called deferred have since shipped. Qualification-gating: `app_private.enforce_shift_assignment_eligibility` fires BEFORE INSERT and before any UPDATE of the employee, facility, unit, shift definition, date or times, over `public.evaluate_shift_assignment_eligibility` -- the same predicate `claim_open_shift` and both sides of a swap run, so roster membership is no longer the only gate. Same-day float: `20260731053000` dropped `shift_assignments_employee_id_shift_date_key`, so an employee can work two shifts in a day and at two facilities; `prevent_shift_assignment_overlap` is the anti-double-booking authority now, and it refuses an overlapping interval rather than a second shift. |
...
everything that drifts toward being a scheduler or an engagement suite is explicitly out. _(Update
2026-07: EHR/clinical capability is no longer out — it is a deliberately-built part of the product;
see the "eMAR / resident clinical records" row above and docs/HIPAA_CLINICAL_DATA.md.)_
