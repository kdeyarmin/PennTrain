...
5. Set Edge Function secrets `STRIPE_SECRET_KEY`, `STRIPE_BILLING_WEBHOOK_SECRET`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SHARED_SECRET`; add the same cron secret to Supabase Vault as `cron_shared_secret`. **The two Stripe secrets are required, and each missing one fails a different way:** without `STRIPE_SECRET_KEY` both Checkout and the Customer Portal answer `503 billing_not_configured` and the hourly `billing-quantity-sync` job answers `503 billing_sync_not_configured`; without `STRIPE_BILLING_WEBHOOK_SECRET` the webhook rejects **every** Stripe event with `400 invalid_signature`, so subscriptions, invoices, and billing state never reconcile. Also set `BILLING_RETURN_URL_ORIGINS` to the exact production and approved staging origins. It is not strictly required -- `resolvePhase2BillingReturnOrigins` falls back through `PUBLIC_APP_URL` and `SIGNUP_REDIRECT_ORIGINS` to a `https://cmcarebase.com` default -- but unset means the allowlist is whatever that chain resolves to, and a deploy served from any other origin has its Checkout and Portal redirects refused with `400 invalid_return_url` (the caller-controlled request `Origin` header never extends the allowlist). Verify the set with `supabase secrets list` before step 6. A missing `STRIPE_SECRET_KEY` now surfaces on its own — `sync-billing-quantities` records a failed run hourly and the watchdog reports it stale — but a missing `STRIPE_BILLING_WEBHOOK_SECRET` does not: nothing schedules the webhook, so there is no run to fail, and the first symptom is real subscription traffic silently failing to reconcile. Send a test event from the Stripe dashboard rather than waiting to find out.
6. Run Checkout for Train and CareBase (monthly and annual). Verify the subscription item quantity is **1** and the invoice amount matches the flat fee.
7. Configure and verify the Stripe webhook endpoint before accepting live Checkout sessions, then repeat with live Price IDs on an internal organization.
8. Confirm the **Billing quantity synchronization** system job still runs; for flat plans it should keep quantity at 1.
