...
5. Set Edge Function secrets `STRIPE_SECRET_KEY`, `STRIPE_BILLING_WEBHOOK_SECRET`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SHARED_SECRET`; add the same cron secret to Supabase Vault as `cron_shared_secret`, and set `BILLING_RETURN_URL_ORIGINS` to the exact production and approved staging origins. **Every one of these is required, and each missing one fails a different way:** without `STRIPE_SECRET_KEY` both Checkout and the Customer Portal answer `503 billing_not_configured` and the hourly `billing-quantity-sync` job answers `503 billing_sync_not_configured`; without `STRIPE_BILLING_WEBHOOK_SECRET` the webhook rejects **every** Stripe event with `400 invalid_signature`, so subscriptions, invoices, and billing state never reconcile; without `BILLING_RETURN_URL_ORIGINS` the allowlist is empty and every Checkout and Portal request is refused with `400 invalid_return_url` (the request `Origin` header deliberately does not extend the allowlist). Verify the set with `supabase secrets list` before step 6 — a missing secret does not surface at deploy time, only at the first customer's checkout.
