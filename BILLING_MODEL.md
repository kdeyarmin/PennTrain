...
5. Set Edge Function secrets `STRIPE_SECRET_KEY`, `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, and `CRON_SHARED_SECRET`; add the same cron secret to Supabase Vault as `cron_shared_secret`, and set `BILLING_RETURN_URL_ORIGINS` to the exact production and approved staging origins.
