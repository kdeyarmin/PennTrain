...
| **WP8** | `platform_admin` · production, before 2026-09-12 | Reconcile the stranded LEIE run → "Run now" the monthly refresh → watch the run heartbeat through staging | A new `oig_leie` snapshot activates; `exclusion_source_health` reads `healthy`; no second `staging` row is left behind. (At risk today: I27, I28) |
