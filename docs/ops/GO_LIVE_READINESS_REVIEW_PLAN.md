...
| N1 / G270 | **All six closed, in two steps.** `20260904050000` moved four cron entries onto the definition its Edge Function actually finishes. The other two recorded no runs at all, so they needed code first: `run-data-lifecycle` and `process-organization-export-jobs` now claim and finish a run, and `20260904090000` repoints and relabels them. The query that defines the gap now returns zero rows | `20260904050000`, `20260904090000`, G270/H17 |
