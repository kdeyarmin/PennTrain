...
const payload = { domain: "course_block" as const, organizationId: "org-1", profileId: "user-1", versionId: "v1", data: { title: "Safe lesson", body: "Employee training content" } };
describe("offline learning safety", () => {
  it("accepts only scoped employee training content", () => expect(() => assertOfflinePayloadAllowed(payload)).not.toThrow());
