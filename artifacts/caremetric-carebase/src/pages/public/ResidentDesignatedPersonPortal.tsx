...
export default function ResidentDesignatedPersonPortal() {
  ...
  const scheduleResponseLabel = respondToSchedule.variables?.response === "confirmed"
    ? "confirmed"
    : respondToSchedule.variables?.response === "needs_change"
      ? "change request"
  ...
  return <div className="min-h-screen bg-muted/30 px-4 py-10"><div className="mx-auto max-w-4xl space-y-6">
    ...
    {snapshot.isLoading ? <div className="flex justify-center py-16" role="status"><Loader2 className="h-8 w-8 animate-spin" /><span className="sr-only">Loading secure portal</span></div>
      ...
      : data?.accessStatus === "active" ? <>
        ...
        {data.clinicalDisclosureAllowed === false && (
          <Alert>
            <ShieldCheck className="h-4 w-4" />
            <AlertTitle>Document sharing limited</AlertTitle>
            <AlertDescription>
              Shared clinical documents and appointment preparation details are withheld for this resident
              until the facility records disclosure consent. Schedule times, messages, and finance summaries
              remain available under this grant.
            </AlertDescription>
          </Alert>
        )}
