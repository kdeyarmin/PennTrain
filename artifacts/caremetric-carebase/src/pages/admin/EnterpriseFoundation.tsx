...
function RegulatoryExpansionPanel() {
  ...
  const installTemplate = async (templateKey: string, name: string) => {
    ...
    try {
      ...
      toast({ title: `${name} could not be installed`, description: error instanceof Error ? error.message : String(error), variant: "destructive" });
        ...
        <div>
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3">
              <div><p className="font-medium">PA PCH personnel rule pack</p><p className="text-xs text-muted-foreground">55 Pa. Code Ch. 2600 — PCH personnel training. Counsel-cleared template; install, complete the guarded workflow, and activate to enable governed PA coverage.</p></div>
              <Button variant="outline" onClick={() => void installTemplate("pa.pch.2600.65.personnel", "PA PCH personnel rule pack")} disabled={!!installing}>{installing === "pa.pch.2600.65.personnel" ? "Installing draft..." : "Install PA PCH draft"}</Button>
            </div>
            <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3">
              <div><p className="font-medium">PA ALF personnel rule pack</p><p className="text-xs text-muted-foreground">55 Pa. Code Ch. 2800 — ALF personnel training. Counsel-cleared template; install, complete the guarded workflow, and activate to enable governed PA coverage.</p></div>
              <Button variant="outline" onClick={() => void installTemplate("pa.alf.2800.65.personnel", "PA ALF personnel rule pack")} disabled={!!installing}>{installing === "pa.alf.2800.65.personnel" ? "Installing draft..." : "Install PA ALF draft"}</Button>
            </div>
          </div>
        </div>
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border p-3">
          ...
          <Button onClick={() => void installTemplate("oh.rcf.3701-16.personnel", "Ohio rule pack")} disabled={!!installing}>{installing === "oh.rcf.3701-16.personnel" ? "Installing draft..." : "Install Ohio draft"}</Button>
        </div>
        <div>
