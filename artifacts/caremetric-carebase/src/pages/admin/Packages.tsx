      ...
      <Dialog open={showPackageForm} onOpenChange={(open) => { if (!open) { setShowPackageForm(false); setEditId(null); setPackageForm(EMPTY_PACKAGE_FORM); } }}>
        <DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto">
          ...
          <div className="grid grid-cols-1 gap-4 py-2 sm:grid-cols-2">
            ...
            <div className="col-span-full space-y-1.5"><Label id={`${__fieldIds}-caremetric-products`}>CareMetric products</Label><div className="space-y-2 rounded-lg border p-3" role="group" aria-labelledby={`${__fieldIds}-caremetric-products`}>{PRODUCT_MODULES.map((module) => {
              ...
              return <div key={module.id} className="flex items-center justify-between gap-4"><div><p className="text-sm font-medium">{module.name}</p><p className="text-xs text-muted-foreground">{module.description}</p></div><Switch id={`${__fieldIds}-module-${module.id}`} checked={enabled} disabled={forcedByCareBase} onCheckedChange={(checked) => packageField("enabledModules", checked ? (module.id === "carebase" ? [...ALL_PURCHASABLE_PRODUCT_MODULE_IDS] : Array.from(new Set([...packageForm.enabledModules, module.id]))) : packageForm.enabledModules.filter((moduleId) => moduleId !== module.id))} aria-label={`${enabled ? "Disable" : "Enable"} ${module.name}`} /></div>;
