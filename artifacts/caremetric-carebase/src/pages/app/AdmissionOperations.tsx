      ...
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3 border-t pt-4">
          ...
          <div className="grid gap-2 sm:grid-cols-[180px_1fr_auto]">
            ...
            <Button variant="outline" disabled={activity.isPending || (!activityNotes.trim() && !activityDate)} onClick={() => prospect && activity.mutate({ prospectId: prospect.id, activityType, notes: activityNotes, outcome: activityNotes, scheduledFor: activityDate ? new Date(activityDate).toISOString() : undefined }, { onSuccess: () => { toast({ title: "Activity recorded" }); setActivityNotes(""); setActivityDate(""); }, onError: (error: Error) => toast({ title: "Couldn't record activity", description: error.message, variant: "destructive" }) })}>Add activity</Button>
          </div>
