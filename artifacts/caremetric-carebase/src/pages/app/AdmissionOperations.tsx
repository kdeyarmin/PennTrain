...
export default function AdmissionOperations() {
  ...
  // Residents whose bed is still held by facility_beds, whatever the resident row says. A discharge
  // recorded outside the census workflow (which is what the resident page used to do) leaves exactly
  // that: status 'discharged' with the bed still 'occupied' and occupied_by_resident_id set.
  const residentIdsHoldingABed = useMemo(
    () => new Set((beds.data ?? []).map(bed => bed.occupied_by_resident_id).filter((value): value is string => Boolean(value))),
    [beds.data],
  );
  // Pre-admission residents stay out -- admitting is complete_move_in_admission's job. Discharged and
  // deceased residents normally stay out too, but one still holding a bed is a state only this
  // control can repair: re-running the transition releases the bed and writes the census event the
  ...
  return (
    <div className="space-y-6">
      ...
      {(prospects.isError || beds.isError || workspaces.isError || residents.isError || censusEvents.isError) && (
        <QueryError
          what="admissions metrics"
          error={(prospects.error ?? beds.error ?? workspaces.error ?? residents.error ?? censusEvents.error) as Error}
          onRetry={() => void Promise.all([prospects.refetch(), beds.refetch(), workspaces.refetch(), residents.refetch(), censusEvents.refetch()])}
        />
      )}

      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {metrics.map(({ label, value, icon: Icon, className }) => <Card key={label}><CardContent className="flex items-center gap-3 pt-6"><Icon className={`h-7 w-7 ${className}`} /><div><p className="text-2xl font-bold">{metricsLoading || metricsBlocked ? "—" : value}</p><p className="text-sm text-muted-foreground">{label}</p></div></CardContent></Card>)}
      </div>
