...
const YearlyTrainingPlans = lazy(() => import("./TrainingPlans"));
...
export default function TrainWorkspace() {
  ...
  async function submit(kind: string, event: FormEvent<HTMLFormElement>) {
    ...
    if (kind === "plan") payload.requirement_keys = fields.getAll("requirement_keys").map(String);
    try {
  ...
  function reportCsv() {
    const report: unknown[][] = [["Training evidence readiness (not facility compliance certification)", facility?.name, today],
      ["Student", "Employee ID", "Requirement", "Rule", "Status", "Regulatory reference date", "Evidence / action"]];
    ...
    report.push([], ["Scheduled instruction", "Student ID", "Duties", "Scheduled time", "Minutes", "Location", "Fulfillment evidence"]);
  ...
  return <div className="space-y-6" id="train-workspace">
    ...
    <Tabs value={tab} onValueChange={setTab}>
      ...
      <TabsContent value="students" className="space-y-4">
        ...
        {canWrite ? <form key={`${student}-${profile?.first_work_date}`} onSubmit={e => void submit("profile", e)} className="grid gap-4 max-w-xl">
          ...
          <Options name="specialty_unit" label="Specialty unit" value={profile?.specialty_unit} options={{ none: "None", pch_dementia: "PCH secured dementia unit", alr_dementia: "ALF dementia special care", alr_inrbi: "ALF INRBI special care" }} />
          ...
          <p className="text-sm">Roster hire date: {chosen.hire_date || "Missing"}. The confirmed training hire date determines ALF 30-day deadlines and employment anniversary years. Explain corrections in the duty description.</p>
      ...
      <TabsContent value="evidence" className="space-y-4">{!chosen ? <p>Choose a student to record or review training evidence.</p> : <>
      ...
      <TabsContent value="reports" className="space-y-4">
        ...
        {rows.map(({ employee, checks }) => <section key={employee.id} className="break-inside-avoid"><h3 className="font-semibold mt-5">{employee.first_name} {employee.last_name}</h3><table className="w-full text-sm"><thead><tr className="text-left"><th>Requirement</th><th>Evidence status / reference date</th><th>Evidence or action</th></tr></thead><tbody>{checks.map(c => <tr key={c.key} className="border-t align-top"><td className="p-2">{c.label}<br /><span className="text-muted-foreground">55 Pa. Code {c.citation}</span></td><td className="p-2">{c.status}<br />{c.due}</td><td className="p-2">{c.detail}</td></tr>)}</tbody></table>
          ...
          <h4 className="font-semibold mt-4">Scheduled instruction and fulfillment</h4>
    ...
    <style>{`@media print { @page { size: letter; margin: 12mm; } html, body, #root, #root > div, main#main-content { height: auto !important; max-height: none !important; overflow: visible !important; position: static !important; } body * { visibility: hidden; overflow: visible !important; } #train-workspace, #train-workspace * { visibility: visible; } #train-workspace { position: absolute; top: 0; left: 0; right: 0; padding: 0; font-size: 10pt; } #train-workspace section { break-inside: auto; } #train-workspace h3, #train-workspace h4 { break-after: avoid; } #train-workspace table { table-layout: fixed; width: 100%; font-size: 10pt; } #train-workspace th:nth-child(1) { width: 28%; } #train-workspace th:nth-child(2) { width: 17%; } #train-workspace th:nth-child(3) { width: 55%; } #train-workspace tr { break-inside: avoid; } #train-workspace th, #train-workspace td { overflow-wrap: break-word; } #train-workspace .print\\:hidden { display: none; } }`}</style>
  </div>;
}
