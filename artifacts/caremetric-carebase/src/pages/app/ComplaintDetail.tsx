  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>Investigation, response & closure</CardTitle><CardDescription>Auditors can review this record; only authorized managers can change it.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2">
        ...
        {canManage && <div className="space-y-1 sm:col-span-2"><Label>Reason for this update *</Label><Input value={reason} onChange={event => setReason(event.target.value)} placeholder="Document the decision or documentation added" /></div>}
      ...
      <div className="grid gap-4 xl:grid-cols-3">
        <Card><CardHeader className="flex-row items-center justify-between"><div><CardTitle className="flex items-center gap-2"><MessageSquareText className="h-5 w-5" />Interviews</CardTitle><CardDescription>Append-only investigation documentation</CardDescription></div>{canManage && <Button size="sm" onClick={() => setInterviewOpen(true)}><Plus className="h-4 w-4" /></Button>}</CardHeader><CardContent className="space-y-3">{!activity.data?.interviews.length ? <p className="text-sm text-muted-foreground">No interviews recorded.</p> : activity.data.interviews.map(item => <div key={item.id} className="rounded border p-3"><p className="font-medium">{item.person_name}</p><p className="text-xs text-muted-foreground">{item.relationship_to_case} · {new Date(item.interviewed_at).toLocaleString()}</p><p className="mt-2 text-sm">{item.notes}</p></div>)}</CardContent></Card>
