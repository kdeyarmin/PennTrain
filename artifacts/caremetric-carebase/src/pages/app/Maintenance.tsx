...
// A function, not a module constant: facilityToday() evaluated at chunk load froze "Next due date"
// at whatever day the tab was opened.
const emptySchedule = () => ({
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={showOrder} onOpenChange={(open) => { if (!open) { setShowOrder(false); setOrderForm(emptyOrder); } else setShowOrder(true); }}><DialogContent className="max-h-[90vh] max-w-2xl overflow-y-auto"><DialogHeader><DialogTitle>New environmental work order</DialogTitle></DialogHeader><div className="grid gap-4 py-2 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2"><Label htmlFor={`${__fieldIds}-problem-description`}>Problem description *</Label><Textarea id={`${__fieldIds}-problem-description`} value={orderForm.description} onChange={(e) => setOrderForm({ ...orderForm, description: e.target.value })} placeholder="Describe what is wrong and what was observed" /></div>
        <div><Label htmlFor={`${__fieldIds}-safety-risk`}>Safety risk</Label><Select value={orderForm.safetyRisk} onValueChange={(value) => setOrderForm({ ...orderForm, safetyRisk: value })}><SelectTrigger id={`${__fieldIds}-safety-risk`}><SelectValue /></SelectTrigger><SelectContent>{["none","low","moderate","high","immediate_danger"].map((v) => <SelectItem key={v} value={v}>{humanize(v)}</SelectItem>)}</SelectContent></Select></div>
        <div><Label htmlFor={`${__fieldIds}-priority`}>Priority</Label><Select value={orderForm.priority} onValueChange={(value) => setOrderForm({ ...orderForm, priority: value })}><SelectTrigger id={`${__fieldIds}-priority`}><SelectValue /></SelectTrigger><SelectContent>{["routine","urgent","emergency"].map((v) => <SelectItem key={v} value={v}>{humanize(v)}</SelectItem>)}</SelectContent></Select></div>
        <div className="sm:col-span-2"><Label htmlFor={`${__fieldIds}-temporary-protective-action`}>Temporary protective action</Label><Textarea id={`${__fieldIds}-temporary-protective-action`} value={orderForm.protectiveAction} onChange={(e) => setOrderForm({ ...orderForm, protectiveAction: e.target.value })} placeholder="Area secured, equipment removed from service, alternate route posted…" /></div>
        <div><EmployeeSearchSelect label="Assigned maintenance employee" value={orderForm.employeeId === "none" ? "" : orderForm.employeeId} onValueChange={(id) => setOrderForm({ ...orderForm, employeeId: id || "none" })} facilityId={orderForm.facilityId || undefined} allowEmpty emptyLabel="Unassigned" emptyValue="none" /></div>
        <div><Label htmlFor={`${__fieldIds}-external-vendor`}>External vendor</Label><Input id={`${__fieldIds}-external-vendor`} value={orderForm.vendor} onChange={(e) => setOrderForm({ ...orderForm, vendor: e.target.value })} /></div>
        <div><Label htmlFor={`${__fieldIds}-target-completion`}>Target completion</Label><Input id={`${__fieldIds}-target-completion`} type="datetime-local" value={orderForm.target} onChange={(e) => setOrderForm({ ...orderForm, target: e.target.value })} /></div>
        <div><Label htmlFor={`${__fieldIds}-estimated-cost`}>Estimated cost</Label><Input id={`${__fieldIds}-estimated-cost`} type="number" min="0" step="0.01" value={orderForm.estimatedCost} onChange={(e) => setOrderForm({ ...orderForm, estimatedCost: e.target.value })} /></div>
