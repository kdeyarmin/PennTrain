...
function ImplementationTaskCard({
  ...
  task: Record<string, any> & ImplementationTaskLike;
  ...
  const route = implementationTaskRoute(task.task_key);
  const attention = implementationTaskNeedsAttention(task);

  useEffect(() => {
    setStatus(task.status);
    setOwnerId(task.owner_profile_id ?? "unassigned");
    setDueOn(task.due_date ?? "");
    setNote(task.evidence_note ?? "");
  }, [task.due_date, task.evidence_note, task.owner_profile_id, task.status]);

  return (
    <Card className={attention ? "border-amber-500/60" : undefined}>
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <div className="flex flex-wrap items-center gap-2">
              <CardTitle className="text-base">{task.title}</CardTitle>
              {task.required !== false && <Badge variant="outline">Required</Badge>}
              {attention && <Badge variant="destructive">Needs attention</Badge>}
            </div>
            <CardDescription className="mt-1">{task.description || human(task.category)}</CardDescription>
          </div>
          {route && (
            <Button asChild size="sm" variant="outline" className="print:hidden">
              <Link href={route}>Validate in CareBase <ArrowRight className="ml-2 h-4 w-4" /></Link>
            </Button>
          )}
        </div>
      </CardHeader>
      <CardContent className="grid gap-3 lg:grid-cols-[180px_220px_160px_1fr_auto] lg:items-end">
            ...
            <SelectContent>
              ...
              {profiles.map(profile => (
                <SelectItem key={profile.id} value={profile.id}>
                  {profile.first_name} {profile.last_name} · {human(profile.role)}
                </SelectItem>
              ))}
            </SelectContent>
          </Select>
        ...
        <Button
          ...
          onClick={() => void onSave({
            taskId: task.id,
            status,
            ownerProfileId: ownerId === "unassigned" ? undefined : ownerId,
...
export default function ValueCenter() {
  ...
  const [automationDescription, setAutomationDescription] = useState(
    "Create accountable work and notify managers when this governed event occurs.",
  );
  ...
  const [automationDueDays, setAutomationDueDays] = useState("7");
  ...
  const [baselineForm, setBaselineForm] = useState<CustomerValueBaselineForm>({
    ...DEFAULT_CUSTOMER_VALUE_BASELINE,
  });
  ...
  const baselineScope = organizationId ?? "current-organization";
  ...
  useEffect(() => {
    ...
    if (!value.data || !baselineVersion || baselineVersion === hydratedBaselineVersion
      || (baselineDirty && !scopeChanged)) return;
  ...
  const currentProject = data?.implementationProjects.find(project => project.status !== "live")
    ?? data?.implementationProjects[0];
  const projectTasks = (currentProject?.tasks ?? []) as Array<Record<string, any> & ImplementationTaskLike>;
  const readiness = summarizeImplementationReadiness(projectTasks);
  ...
  const activeProfiles = (profiles.data ?? []).filter(profile => profile.is_active) as TaskProfile[];

  const quickLinks: Array<{ title: string; description: string; href: string; icon: LucideIcon }> = [
    {
      title: "Daily work",
      description: "Role-based priorities, exceptions, shifts, and work items in one queue.",
      href: "/app/today",
      icon: Activity,
    },
    {
      title: "Inspection readiness",
      description: "Convert readiness work into a documentation-backed response room.",
      href: "/app/inspection-readiness",
      icon: ClipboardCheck,
    },
    {
      title: "Governed assistant",
      description: "Ground responses in citations and send proposed work through approval.",
      href: "/app/regulatory-copilot",
      icon: Bot,
    },
  ];

  const saveImplementationTask = async (input: {
    taskId: string;
    status: string;
    ownerProfileId?: string;
    dueOn?: string;
    ...
    await notify(() => updateImplementationTask.mutateAsync(input), "Implementation task updated");
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertTitle>One governed operating system</AlertTitle>
        <AlertDescription>
          Automation creates traceable work, the inspection room links requests to documentation,
          assistant actions require human approval, and every workflow remains tenant- and facility-scoped.
        </AlertDescription>
      </Alert>
      ...
      <Tabs defaultValue="overview" className="space-y-5">
        <TabsList className="flex h-auto flex-wrap justify-start print:hidden">
          ...
          <TabsTrigger value="inspection">Inspection room</TabsTrigger>
          <TabsTrigger value="implementation">Go-Live Center</TabsTrigger>
          <TabsTrigger value="integrations">Integrations</TabsTrigger>
          <TabsTrigger value="reports">Savings & reports</TabsTrigger>
          <TabsTrigger value="workforce">Staffing</TabsTrigger>
          <TabsTrigger value="operations">Admissions & operations</TabsTrigger>
        </TabsList>

        <TabsContent value="overview" className="space-y-5">
          {value.isLoading ? (
            <QueryLoading what="customer value metrics" />
          ) : value.isError ? (
            <QueryError what="customer value metrics" error={value.error} onRetry={() => value.refetch()} />
          ) : (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Metric
                ...
                value={data?.automations.filter(rule => rule.state === "active").length ?? 0}
                detail={`${number(data?.automationRuns.length)} recent execution receipts`}
                icon={Sparkles}
              />
              <Metric label="Open inspection rooms" value={openWarRooms.length} detail="Live response workspaces" icon={Radar} />
              <Metric
                label="Go-live readiness"
                value={currentProject ? `${readiness.percent}%` : "—"}
                detail={currentProject ? `${readiness.launchBlockers} required milestone(s) remain` : "Start an implementation project"}
                icon={FileCheck2}
              />
            </div>
          )}
          <div className="grid gap-4 lg:grid-cols-3">
            {quickLinks.map(({ title, description, href, icon: Icon }) => (
              <Card key={title}>
                ...
                <CardContent><Button asChild variant="outline"><Link href={href}>Open workflow</Link></Button></CardContent>
              </Card>
            ))}
          </div>
        </TabsContent>
        ...
        <TabsContent value="automation" className="space-y-5">
          <Card>
            <CardHeader>
              <CardTitle>Governed automation builder</CardTitle>
              <CardDescription>
                Configure an allowlisted trigger and two safe actions: create accountable work and notify
                authorized roles. Save as a draft before activation when review is required.
              </CardDescription>
            </CardHeader>
            <CardContent className="grid gap-4 md:grid-cols-6">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  min={0}
                  max={365}
                  value={automationDueDays}
                  onChange={event => setAutomationDueDays(event.target.value)}
                />
              </div>
              ...
              <div className="flex items-end">
                <Button
                  ...
                  disabled={
                    ...
                    || automationDescription.trim().length < 3
                    || !Number.isInteger(Number(automationDueDays))
                    || Number(automationDueDays) < 0
                    || Number(automationDueDays) > 365
                    || saveAutomation.isPending
                  }
                  onClick={() => void notify(() => saveAutomation.mutateAsync({
                    facilityId,
                    ...
                    actions: [
                      {
                        ...
                        dueDays: Number(automationDueDays),
                      },
                      {
                        type: "notify_roles",
                        title: automationName.trim(),
                        body: automationDescription.trim(),
                        roles: ["org_admin", "facility_manager"],
                        link: "/app/today",
                      },
                    ],
                    state: automationState,
                  }), automationState === "active" ? "Automation activated" : "Automation draft saved")}
                >
                  <Sparkles className="mr-2 h-4 w-4" />
                  {automationState === "active" ? "Activate" : "Save draft"}
                </Button>
              </div>
            </CardContent>
          </Card>
          ...
          <div className="space-y-3">
            {data?.automations.length ? data.automations.map(rule => {
                  ...
                  <CardContent className="flex flex-wrap items-center justify-between gap-3 p-4">
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <Badge variant={rule.state === "active" ? "default" : "secondary"}>{human(rule.state)}</Badge>
                      </div>
                      <p className="text-sm text-muted-foreground">
                        {human(rule.trigger_type)} · {number(rule.run_count)} executions
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() => void notify(() => saveAutomation.mutateAsync({
                          ruleId: rule.id,
                          facilityId: rule.facility_id ?? undefined,
                          name: rule.name,
                          ...
                          state: nextState,
                        }), nextState === "active" ? "Automation activated" : "Automation paused")}
                      >
                        {nextState === "active" ? "Activate" : "Pause"}
                      </Button>
                      <Button
                        size="sm"
                        variant="outline"
            ...
            }) : <Empty>No automation rules yet. Save the first governed rule above.</Empty>}
          </div>
        </TabsContent>
        ...
        <TabsContent value="inspection" className="space-y-5">
          <Card>
            <CardHeader>
              <CardTitle>Start an inspection response room</CardTitle>
              <CardDescription>
                Coordinate a survey, complaint response, or mock inspection with numbered documentation
                requests and linked operational work.
              </CardDescription>
            </CardHeader>
            <CardContent className="flex flex-wrap items-end gap-3">
              ...
              <Button
                ...
                onClick={() => void notify(() => createWarRoom.mutateAsync({
                  facilityId,
                  name: warRoomName.trim(),
                  inspectionType: "routine",
                  notes: "Created from CareBase Value Center",
                }), "Inspection room opened")}
              >
                <Plus className="mr-2 h-4 w-4" /> Open room
              </Button>
            </CardContent>
          </Card>

          {openWarRooms.length ? openWarRooms.map(room => (
            <Card key={room.id}>
              <CardHeader>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <CardTitle>{room.name}</CardTitle>
                    <CardDescription>{human(room.inspection_type)} · {human(room.status)}</CardDescription>
                  </div>
                  <Button
                    variant="outline"
                    size="sm"
                    disabled={addWarRoomRequest.isPending}
                    onClick={() => void notify(() => addWarRoomRequest.mutateAsync({
                      warRoomId: room.id,
              ...
              <CardContent className="space-y-2">
                {room.requests?.length ? room.requests.map((request: Record<string, any>) => (
                  <div key={request.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3">
                    <div>
                      <p className="font-medium">#{request.request_number} {request.title}</p>
                      <p className="text-xs text-muted-foreground">
                        {human(request.status)} · due {request.due_at ? new Date(request.due_at).toLocaleString() : "not set"}
                      </p>
                    </div>
                    {request.status !== "closed" && (
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={updateWarRoomRequest.isPending}
                        onClick={() => void notify(() => updateWarRoomRequest.mutateAsync({
                          requestId: request.id,
                          status: "closed",
                          responseNote: "Documentation verified by inspection lead.",
                        }), "Request verified and closed")}
                      >
                        <CheckCircle2 className="mr-2 h-4 w-4" /> Verify
                      </Button>
                    )}
                  </div>
                )) : <Empty>No requests yet. Add the first documentation request.</Empty>}
              </CardContent>
            </Card>
          )) : <Empty>No active inspection rooms.</Empty>}
        </TabsContent>
        ...
        <TabsContent value="implementation" className="space-y-5">
          {!currentProject && (
            <Card>
              <CardHeader>
                <CardTitle>Start the Organization Go-Live Center</CardTitle>
                <CardDescription>
                  Create one accountable project for configuration, imports, validation, training,
                  security, survey rehearsal, and production approval.
                </CardDescription>
              </CardHeader>
              <CardContent className="flex flex-wrap items-end gap-3">
                <div className="min-w-72 flex-1 space-y-2">
                ...
                <Button
                  ...
                  onClick={() => void notify(() => initializeProject.mutateAsync({
                    name: projectName.trim(),
                    targetGoLiveOn: through,
                    sourceSystems: ["spreadsheets", "paper_binders", "legacy_point_solutions"],
                  }), "Implementation project initialized")}
                >
                  <FolderKanban className="mr-2 h-4 w-4" /> Start implementation
                </Button>
              </CardContent>
            </Card>
          )}
                ...
                <CardHeader>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <CardDescription>
                        {human(currentProject.status)} · target {currentProject.target_go_live_date
                          ? new Date(`${currentProject.target_go_live_date}T00:00:00`).toLocaleDateString()
                          : "not set"}
                      </CardDescription>
                    </div>
                    <Button variant="outline" className="print:hidden" onClick={() => window.print()}>
                      <Printer className="mr-2 h-4 w-4" /> Print readiness report
                    </Button>
                  </div>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
                    <div className="rounded-md border p-3">
                      ...
                      <p className="text-2xl font-bold">{readiness.percent}%</p>
                    </div>
                    <div className="rounded-md border p-3">
                      <p className="text-xs text-muted-foreground">Required complete</p>
                      <p className="text-2xl font-bold">{readiness.requiredComplete}/{readiness.required}</p>
                    </div>
                    <div className="rounded-md border p-3">
                      <p className="text-xs text-muted-foreground">Launch blockers</p>
                      <p className="text-2xl font-bold">{readiness.launchBlockers}</p>
                    </div>
                    <div className="rounded-md border p-3">
                      <p className="text-xs text-muted-foreground">Blocked</p>
                      <p className="text-2xl font-bold">{readiness.blocked}</p>
                    </div>
                    <div className="rounded-md border p-3">
                      <p className="text-xs text-muted-foreground">Overdue</p>
                      <p className="text-2xl font-bold">{readiness.overdue}</p>
                    </div>
                  </div>
                  ...
                  <Alert variant={readiness.ready ? "default" : "destructive"}>
                    ...
                    <AlertTitle>{readiness.ready ? "Ready for go-live approval" : "Go-live is not ready"}</AlertTitle>
                    <AlertDescription>
                      {readiness.ready
                        ? "Every required milestone is complete or approved as not applicable. Record final approval and cutover evidence."
                        : `${readiness.launchBlockers} required milestone(s) still need completion or an approved not-applicable decision.`}
                    </AlertDescription>
                  </Alert>
                </CardContent>
              </Card>
              ...
              <div className="space-y-3">
                {projectTasks.map(task => (
                  <ImplementationTaskCard
                    ...
                    pending={updateImplementationTask.isPending}
                    onSave={saveImplementationTask}
                  />
                ))}
              </div>
            </>
          ) : (
            <Empty>No implementation project has been started.</Empty>
          )}
        </TabsContent>

        <TabsContent value="integrations" className="space-y-5">
          <div className="grid gap-4 md:grid-cols-3">
            <Metric label="API credentials" value={data?.integration.credentials.length ?? 0} icon={Cable} />
            <Metric label="Webhook endpoints" value={data?.integration.endpoints.length ?? 0} icon={Activity} />
            <Metric label="Failed deliveries" value={data?.integration.deliveryFailures ?? 0} icon={AlertTriangle} />
          </div>
          <Alert>
            <Cable className="h-4 w-4" />
            <AlertTitle>Signed, scoped, and replayable</AlertTitle>
            <AlertDescription>
              The Integration Hub uses one-time API secrets, explicit scopes, signed webhook delivery,
              retry documentation, and dead-letter replay. Medication sources bind to the same governed credentials.
            </AlertDescription>
          </Alert>
          <div className="grid gap-4 lg:grid-cols-2">
            <Card>
              ...
              <CardContent className="space-y-2">
                {data?.integration.credentials.length ? data.integration.credentials.map(item => (
                  <div key={item.id} className="rounded border p-3">
                    <div className="flex justify-between gap-2">
                      <p className="font-medium">{item.name}</p>
                      <Badge variant="outline">{human(item.status)}</Badge>
                    </div>
                    <p className="mt-1 text-xs text-muted-foreground">{(item.scopes ?? []).join(", ")}</p>
                  </div>
                )) : <Empty>No API credentials issued.</Empty>}
              </CardContent>
            </Card>
            <Card>
              <CardHeader><CardTitle>Webhook health</CardTitle></CardHeader>
              <CardContent className="space-y-2">
                {data?.integration.endpoints.length ? data.integration.endpoints.map(item => (
                  <div key={item.id} className="rounded border p-3">
                    <div className="flex justify-between gap-2">
                      <p className="font-medium">{item.name}</p>
                      <Badge variant={Number(item.consecutiveFailures) > 0 ? "destructive" : "outline"}>
                        {human(item.status)}
                      </Badge>
                    </div>
                    <p className="mt-1 text-xs text-muted-foreground">
                      {number(item.consecutiveFailures)} consecutive failures
                    </p>
                  </div>
        ...
        <TabsContent value="reports" className="space-y-5">
          {value.isLoading ? (
            <QueryLoading what="value and savings metrics" />
          ) : value.isError ? (
            <QueryError what="value and savings metrics" error={value.error} onRetry={() => value.refetch()} />
          ) : (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <div className="grid gap-5 xl:grid-cols-2">
            <Card>
              <CardHeader>
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <CardTitle>Value baseline</CardTitle>
                  {!baselineConfigured ? (
                    <Badge variant="secondary">Suggested starting values</Badge>
                  ) : baselineDirty ? (
                    <Badge variant="secondary">Unsaved changes</Badge>
                  ) : (
                    <Badge variant="outline">Saved baseline</Badge>
                  )}
                </div>
                <CardDescription>
                  {baselineConfigured
                    ? "Your saved organization baseline is loaded below. Refreshes will not replace unsaved edits."
                    : "Review these suggested starting values before saving the organization's first baseline."}
                  {" "}Each time estimate is multiplied only by its matching recorded outcome.
                </CardDescription>
              </CardHeader>
                ...
                <div className="flex flex-wrap gap-2 sm:col-span-2">
                  <AlertDialog>
                    <AlertDialogTrigger asChild>
                      <Button disabled={!baselineReady || !baselineValid || !baselineDirty || saveBaseline.isPending}>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        <AlertDialogTitle>
                          {baselineConfigured ? "Replace the saved value baseline?" : "Save this value baseline?"}
                        </AlertDialogTitle>
                        <AlertDialogDescription>
                          {baselineConfigured
                            ? "This replaces the organization's current cost and time-saving assumptions. Future value estimates will use the new values."
                            : "Future value estimates will use these organization-specific cost and time-saving assumptions."}
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        ...
                        <AlertDialogAction
                          ...
                          onClick={() => {
                            const submitted = { ...baselineForm };
                            void notify(async () => {
                              await saveBaseline.mutateAsync(customerValueBaselineToInput(submitted));
                              setSavedBaselineForm(submitted);
                              setBaselineConfigured(true);
                            }, "Value baseline saved");
                          }}
                        >
                          {baselineConfigured ? "Replace baseline" : "Save baseline"}
                        </AlertDialogAction>
                      </AlertDialogFooter>
                    </AlertDialogContent>
                  </AlertDialog>
                  <Button
                    variant="outline"
                    disabled={!baselineConfigured || !baselineDirty || saveBaseline.isPending || !savedBaselineForm}
                    onClick={() => savedBaselineForm && setBaselineForm({ ...savedBaselineForm })}
                  >
                    <RotateCcw className="mr-2 h-4 w-4" /> Reset to saved
                  </Button>
                </div>
                {!baselineValid && (
                  <p className="text-sm text-destructive sm:col-span-2">
                    Use the supported non-negative cost and time ranges and no more than 20 system names
                    (120 characters each).
                  </p>
                )}
              </CardContent>
        ...
        <TabsContent value="workforce" className="space-y-5">
          {!facilityId ? (
            ...
            <QueryLoading what="staffing recommendations" />
          ) : staffing.isError ? (
            <QueryError what="staffing recommendations" error={staffing.error} onRetry={() => staffing.refetch()} />
          ) : (
            <>
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                <Metric label="Open shifts" value={staffing.data?.openShifts ?? 0} icon={Users} />
                <Metric label="Pending time off" value={staffing.data?.pendingTimeOff ?? 0} icon={CalendarClock} />
                <Metric label="Pending swaps" value={staffing.data?.pendingSwaps ?? 0} icon={RefreshCw} />
                <Metric label="Blocked assignments" value={staffing.data?.recentBlockedAssignments ?? 0} detail="Last 30 days" icon={ShieldCheck} />
              </div>
              <Card>
                <CardHeader>
                  <CardTitle>Qualification-aware coverage recommendations</CardTitle>
                  <CardDescription>
                    Coverage uses the active schedule, service workload, availability, time off, swaps,
                    and immutable eligibility decisions.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-3">
                  {staffing.data?.recommendations.length ? staffing.data.recommendations.map(item => (
                    <Button key={item.title} asChild variant="outline" className="h-auto w-full justify-between py-3">
                      <Link href={item.href}>
                        <span>{item.title}</span>
                        <Badge variant={item.priority === "high" ? "destructive" : "secondary"}>{human(item.priority)}</Badge>
                      </Link>
                    </Button>
                  )) : (
                    <div className="flex items-center gap-2 text-sm text-emerald-700">
                      <CheckCircle2 className="h-4 w-4" /> No immediate staffing exception identified for the next 30 days.
                    </div>
                  )}
                </CardContent>
              </Card>
        ...
        <TabsContent value="operations" className="space-y-5">
          {admissions.isLoading ? (
            <QueryLoading what="admissions and occupancy intelligence" />
          ) : admissions.isError ? (
            <QueryError what="admissions and occupancy intelligence" error={admissions.error} onRetry={() => admissions.refetch()} />
          ) : (
            <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <Metric label="Active prospects" value={admissions.data?.pipeline.active ?? 0} icon={Users} />
              <Metric label="Expected move-ins" value={admissions.data?.pipeline.expected30Days ?? 0} detail="Next 30 days" icon={BedDouble} />
              <Metric label="Available beds" value={admissions.data?.occupancy.availableBeds ?? 0} icon={BedDouble} />
              <Metric label="Portal requests" value={data?.portalRequests.length ?? 0} icon={ClipboardCheck} />
            </div>
          )}
          <div className="grid gap-5 xl:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>Admissions and occupancy intelligence</CardTitle>
              ...
              <CardContent className="space-y-2">
                {admissions.data?.referralSources.length ? admissions.data.referralSources.map(source => (
                  <div key={source.source} className="grid grid-cols-4 gap-2 rounded border p-3 text-sm">
                    ...
                    <span>{source.conversion_percent}% converted</span>
                  </div>
                )) : <Empty>No referral-source activity yet.</Empty>}
                <Button asChild variant="outline"><Link href="/app/admissions">Open admissions workspace</Link></Button>
              </CardContent>
            </Card>
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Medication sync exceptions, offline conflicts, and governed assistant drafts stay visible until resolved.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div className="rounded border p-3">
                    <p className="text-xs text-muted-foreground">Medication exceptions</p>
                    <p className="text-2xl font-bold">{data?.medicationExceptions.length ?? 0}</p>
                  </div>
                  <div className="rounded border p-3">
                    <p className="text-xs text-muted-foreground">Offline conflicts</p>
                    <p className="text-2xl font-bold">{data?.offline.syncConflicts ?? 0}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Button asChild variant="outline"><Link href="/app/medication-integration">Medication queue</Link></Button>
                  <Button asChild variant="outline"><Link href="/me/courses">Offline learning</Link></Button>
                </div>
              </CardContent>
            </Card>
          </div>
          <Card>
            <CardHeader>
              <CardTitle>Governed assistant action drafts</CardTitle>
              <CardDescription>
                The assistant can propose work, but only a manager can approve execution. Approval creates
                ordinary, traceable work items.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {pendingDrafts.length ? pendingDrafts.map(draft => (
                <div key={draft.id} className="flex flex-wrap items-start justify-between gap-3 rounded border p-3">
                  <div>
                    <p className="font-medium">{draft.title}</p>
                    <p className="text-sm text-muted-foreground">{human(draft.intent)}</p>
                  </div>
                  <div className="flex gap-2">
                    <Button
                      ...
                      onClick={() => void notify(() => reviewDraft.mutateAsync({
                        draftId: draft.id,
                        decision: "reject",
                        reviewNote: "Rejected in Value Center after human review.",
                      }), "Draft rejected")}
                    >
                      Reject
                    </Button>
                    <Button
                      ...
                      onClick={() => void notify(() => reviewDraft.mutateAsync({
                        draftId: draft.id,
                        decision: "approve",
                        reviewNote: "Approved in Value Center after human review.",
                      }), "Draft approved and work created")}
                    >
                      Approve & create work
                    </Button>
                  </div>
                </div>
              )) : <Empty>No assistant drafts await review.</Empty>}
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
