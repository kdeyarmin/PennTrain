  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="overview" className="space-y-5">
      ...
      <TabsContent value="inspection" className="space-y-5">
        <Card><CardHeader><CardTitle>Start an inspection War Room</CardTitle><CardDescription>Coordinate a survey, complaint response, or mock inspection with numbered evidence requests and linked operational work.</CardDescription></CardHeader><CardContent className="flex flex-wrap items-end gap-3"><div className="min-w-72 flex-1 space-y-2"><Label>Room name</Label><Input value={warRoomName} onChange={(event) => setWarRoomName(event.target.value)} /></div><Button disabled={!facilityId || warRoomName.trim().length < 3 || createWarRoom.isPending} onClick={() => void notify(() => createWarRoom.mutateAsync({ facilityId, name: warRoomName.trim(), inspectionType: "routine", notes: "Created from CareBase Value Center" }), "Inspection room opened")}><Plus className="mr-2 h-4 w-4" />Open room</Button></CardContent></Card>
        {openWarRooms.length ? openWarRooms.map((room) => <Card key={room.id}><CardHeader><div className="flex flex-wrap items-center justify-between gap-2"><div><CardTitle>{room.name}</CardTitle><CardDescription>{human(room.inspection_type)} · {human(room.status)}</CardDescription></div><Button variant="outline" size="sm" disabled={addWarRoomRequest.isPending} onClick={() => void notify(() => addWarRoomRequest.mutateAsync({ warRoomId: room.id, title: `Evidence request ${(room.requests?.length ?? 0) + 1}`, description: "Collect, verify, and attach the requested inspection evidence.", priority: "high", dueAt: new Date(Date.now() + 48 * 60 * 60 * 1000).toISOString() }), "Evidence request added")}><Plus className="mr-2 h-4 w-4" />Add request</Button></div></CardHeader><CardContent className="space-y-2">{room.requests?.length ? room.requests.map((request: Record<string, any>) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3"><div><p className="font-medium">#{request.request_number} {request.title}</p><p className="text-xs text-muted-foreground">{human(request.status)} · due {request.due_at ? new Date(request.due_at).toLocaleString() : "not set"}</p></div>{request.status !== "closed" && <Button size="sm" variant="outline" disabled={updateWarRoomRequest.isPending} onClick={() => void notify(() => updateWarRoomRequest.mutateAsync({ requestId: request.id, status: "closed", responseNote: "Evidence verified by inspection lead." }), "Request verified and closed")}><CheckCircle2 className="mr-2 h-4 w-4" />Verify</Button>}</div>) : <Empty>No requests yet. Add the first evidence request.</Empty>}</CardContent></Card>) : <Empty>No active inspection rooms.</Empty>}
      </TabsContent>
