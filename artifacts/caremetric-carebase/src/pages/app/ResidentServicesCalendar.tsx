...
import { addFacilityCalendarDays, facilityDateRangeBounds, facilityDateTimeLocalToUtcIso, facilityToday, formatDateForDisplay, formatFacilityTimeForDisplay, toFacilityDateTimeLocal } from "@/lib/dateUtils";
...
// ordering check alone passed and the handler reached `new Date("").toISOString()` -- a RangeError
// thrown out of an onClick, which is a blank page rather than a validation message. Same shape on
...
export default function ResidentServicesCalendar() {
  ...
  return <div className="space-y-6">
    ...
    <Tabs defaultValue="agenda" className="space-y-4"><TabsList><TabsTrigger value="agenda"><CalendarDays className="mr-2 h-4 w-4" />Agenda</TabsTrigger><TabsTrigger value="vehicles"><Car className="mr-2 h-4 w-4" />Transportation fleet</TabsTrigger></TabsList>
      ...
      <TabsContent value="vehicles"><VehicleWorkspace facilityId={facilityId} vehicles={vehicles.data ?? []} vehiclesLoading={vehicles.isLoading} vehiclesError={vehicles.isError} canManage={canManage} /></TabsContent>
    </Tabs>
...
function CreateEventDialog({ open, onOpenChange, residents, employees, vehicles }: { open: boolean; onOpenChange: (open: boolean) => void; residents: any[]; employees: any[]; vehicles: any[] }) {
  ...
  const submit = () => {
    ...
    mutation.mutate({ residentId: form.residentId, event: { eventType: form.eventType, title: form.title, providerName: form.provider, providerContact: form.providerContact, locationName: form.location, locationAddress: form.address, startsAt: facilityDateTimeLocalToUtcIso(form.starts), endsAt: facilityDateTimeLocalToUtcIso(form.ends), transportationMode: form.transport, vehicleId: form.vehicleId || null, transportationVendor: form.vendor, requiredRecords: list(form.records), preparationInstructions: form.preparation, notes: form.notes }, staff }, { onSuccess: () => { toast({ title: "Resident service scheduled" }); setForm(emptyForm()); onOpenChange(false); }, onError: (error: Error) => toast({ title: "Could not schedule service", description: error.message, variant: "destructive" }) });
  };
...
function OutcomeDialog({ event, onClose, profiles }: { event: ResidentServiceCalendarEventView | null; onClose: () => void; profiles: any[] }) {
  ...
  useEffect(() => { if (event) { setStatus("completed"); setReason(""); setInstructions(""); setNext(""); setFollowUps([]); setDraft({ title: "", description: "", owner: "", due: facilityDefaultAt(3, "17:00"), priority: "high" }); } }, [event]);
...
function VehicleWorkspace({ facilityId, vehicles, vehiclesLoading = false, vehiclesError = false, canManage }: { facilityId: string; vehicles: any[]; vehiclesLoading?: boolean; vehiclesError?: boolean; canManage: boolean }) {
  const { toast } = useToast();
