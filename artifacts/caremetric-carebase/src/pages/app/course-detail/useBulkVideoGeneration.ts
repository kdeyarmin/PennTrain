...
import { hasPendingCourseVideoGeneration } from "@/lib/courseVideoGeneration";

export type BulkVideoGenStatus = "queued" | "confirming" | "processing" | "completed" | "failed";
...
export function useBulkVideoGeneration(blocks: CourseBlock[] | undefined) {
  ...
  const getBulkVideoGenStatus = (block: CourseBlock | undefined, blockId: string): BulkVideoGenStatus => {
    ...
    if (heygenStatus === "submitting" || heygenStatus === "unknown") return "confirming";
    if (heygenStatus === "reconciliation_required") return "failed";
