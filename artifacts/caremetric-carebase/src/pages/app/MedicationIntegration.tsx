  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div><h1 className="text-2xl font-bold tracking-tight">Medication Integration</h1><p className="text-muted-foreground">Monitor read-only eMAR synchronization, resident matching, and non-routine administration evidence.</p></div>
      ...
      <Card><CardContent className="p-4"><div className="max-w-sm space-y-2"><Label>Facility</Label><Select value={facilityId} onValueChange={(value) => { setSelectedFacilityId(value); residentContext.setFacilityId(value); }}><SelectTrigger><SelectValue placeholder="Select facility" /></SelectTrigger><SelectContent>{facilities.data?.map((facility) => <SelectItem key={facility.id} value={facility.id}>{facility.name}</SelectItem>)}</SelectContent></Select>{residentContext.residentId && <p className="text-xs text-muted-foreground">Showing medication evidence for {residentNames.get(residentContext.residentId) ?? "the selected resident"}. Change facility to clear this resident filter.</p>}</div></CardContent></Card>
      ...
      {workspace.isError ? <QueryError what="medication integration" error={workspace.error} onRetry={() => workspace.refetch()} /> : workspace.isLoading ? <QueryLoading what="medication integration" /> : (
        <>
          ...
          <Tabs defaultValue="exceptions"><TabsList><TabsTrigger value="exceptions">Exceptions ({openExceptions.length})</TabsTrigger><TabsTrigger value="orders">External orders</TabsTrigger><TabsTrigger value="administrations">Administration evidence</TabsTrigger></TabsList>
