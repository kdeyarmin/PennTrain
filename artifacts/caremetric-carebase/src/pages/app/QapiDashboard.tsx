...
export default function QapiDashboard() {
  ...
  const [title, setTitle] = useState(""),
    ...
    [source, setSource] = useState("Incident and operational trend"),
  ...
  const submit = () =>
    create.mutate(
      ...
      },
      {
        ...
        onError: (e: Error) =>
          toast({
            title: "Could not create QAPI project",
        ...
        <>
            ...
            {metricEntries.map(([k, v]) => (
              <Card key={k}>
                <CardContent className="pt-5">
                  ...
                  <p className="text-xs text-muted-foreground">{human(k)}</p>
                </CardContent>
              </Card>
            ))}
          </div>
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Problem statement, RCA, interventions, action items,
                measurement, sustainment, and closure approval.
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {!projects.data?.length ? (
                ...
                projects.data.map((p) => (
                  <div
                    key={p.id}
                    className="grid gap-3 rounded-lg border p-4 md:grid-cols-[1fr_180px_160px_auto] md:items-center"
                  >
                    <div>
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        Target{" "}
                        {new Date(
                          `${p.target_completion_date}T00:00:00`,
                        ).toLocaleDateString()}
                      </p>
                    </div>
                    <Badge variant="outline">{human(p.status)}</Badge>
                    <Button asChild size="sm">
                      <Link href={`/app/qapi/projects/${p.id}`}>
                        Open
