  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Drafting aid over facility-scoped CareBase documentation and any active governed rule versions. PA personnel templates are counsel-cleared and installable, but until an active PA governed version is present, answers remain drafting aid rather than compliance determinations. Every answer carries its source, gaps, and authority label.</p>
      </div>

      <Alert>
        <LockKeyhole className="h-4 w-4" />
        <AlertTitle>Drafting aid only — human confirmation remains mandatory</AlertTitle>
        <AlertDescription>PA personnel templates are counsel-cleared and installable, but no active PA governed rule pack is installed yet. Until a PA version completes install, independent review, shadow, and activation, answers remain drafting aid. The Ohio template demonstrates mechanism only and does not cover Pennsylvania. The copilot cannot close findings, approve plans, change resident records, decide incident reportability, invent citations, use superseded rules as current, or alter staffing eligibility.</AlertDescription>
      </Alert>
...
function CopilotResult({ result, disposition, evidenceById, canCreateDraft, creatingDraft, onCreateDraft }: { result: CopilotResultData; disposition?: ReactNode; evidenceById: Map<string, CopilotEvidence>; canCreateDraft: boolean; creatingDraft: boolean; onCreateDraft: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Regulatory sources</CardTitle><CardDescription>Only exact governed rule versions validated against the model’s source IDs.</CardDescription></CardHeader><CardContent className="space-y-3">{result.ruleSources.length === 0 ? <p className="text-sm text-muted-foreground">No active PA governed rule pack is installed. Install and activate a PA PCH or PA ALF pack in Enterprise Foundation to ground answers in governed rule text; until then, answers use facility documentation only and remain drafting aid — not a compliance determination. See missing information.</p> : result.ruleSources.map((source) => <RuleSource key={source.id} source={source} />)}</CardContent></Card>
