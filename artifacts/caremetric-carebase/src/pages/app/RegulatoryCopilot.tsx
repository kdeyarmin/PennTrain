  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertTitle>No active PA governed rule pack today</AlertTitle>
        <AlertDescription>
          If no governed sources are cited below, there is no active Pennsylvania governed rule pack for
          this question today. Treat the response as drafting aid only and verify against source text.
        </AlertDescription>
      </Alert>
...
function CopilotResult({ result, disposition, evidenceById, canCreateDraft, creatingDraft, onCreateDraft }: { result: CopilotResultData; disposition?: ReactNode; evidenceById: Map<string, CopilotEvidence>; canCreateDraft: boolean; creatingDraft: boolean; onCreateDraft: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-2">
      <Card><CardHeader><CardTitle>Regulatory sources</CardTitle><CardDescription>Only exact governed rule versions validated against the model’s source IDs.</CardDescription></CardHeader><CardContent className="space-y-3">{result.ruleSources.length === 0 ? <p className="text-sm text-muted-foreground">No matching active governed rule version was available. There is no active PA governed rule pack for this question today, so treat the response as drafting aid only and verify against source text.</p> : result.ruleSources.map((source) => <RuleSource key={source.id} source={source} />)}</CardContent></Card>
