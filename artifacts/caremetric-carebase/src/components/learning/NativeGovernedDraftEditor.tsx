...
export function NativeGovernedDraftEditor({ versionId, userId, onGovernedChange, onDirtyChange }: { versionId: string; userId: string; onGovernedChange: (value: boolean) => void; onDirtyChange: (value: boolean) => void }) {
  ...
  useEffect(() => { onGovernedChange(!!sourceQuery.data); }, [sourceQuery.data, onGovernedChange]);
  ...
  const run = async (action: 'learning.patchDraft' | 'learning.reviewDraft') => {
    if (!source || (action === 'learning.reviewDraft' && (!reviewed || dirty))) return;
  ...
  return <Card><CardHeader><CardTitle>Governed draft editing and review</CardTitle>
    <CardDescription>Edits and review apply to this exact source revision. Media, quizzes, credits and package acceptance keep their native controls.</CardDescription>
    ...
    <label className="block text-sm">Existing block
      <select className="mt-1 w-full rounded border bg-background p-2" value={blockId} disabled={busy || dirty} onChange={e => selectBlock(e.target.value)}>
    ...
    {source.document.version.aiGenerated && <label className="flex items-start gap-2 text-sm"><input type="checkbox" checked={reviewed} disabled={busy || dirty} onChange={e => setReviewed(e.target.checked)} />
