...
export function OptionalRefreshers({ completedAssignments }: { completedAssignments: { id: string; course_id: string; status: string }[] }) {
  ...
  return <Card><CardHeader><CardTitle>Optional quick refreshers</CardTitle></CardHeader><CardContent className="space-y-3">
    ...
    {feed.data?.lessons.map(item => <div key={item.id} className="flex items-center justify-between gap-3 rounded border p-3"><div><p className="font-medium">{item.title}</p><p className="text-sm text-muted-foreground">About {item.minutes} minutes{item.course_id ? " · Follow-up to a completed course" : ""}</p></div><Button variant="outline" onClick={() => { setLesson(item); setChoice(null); setAnswer(null); }}>Open refresher</Button></div>)}
