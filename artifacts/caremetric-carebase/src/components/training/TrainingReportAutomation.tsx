...
export function TrainingReminderForm({ facilityId, data }: { facilityId: string; data: TrainingAutomation }) {
  ...
  return <form className="space-y-3" onSubmit={e => { e.preventDefault(); save.mutate(settings, { onSuccess: () => toast({ title: "Training reminders updated" }) }); }}>
    <fieldset disabled={save.isPending} className="space-y-3">
      ...
      <div role="status" aria-label="Reminder preview" className="rounded-md border bg-muted/40 p-3 text-sm space-y-1">
        <p className="font-medium">Example with these settings</p>
        <p>{settings.learner_enabled ? `For an incomplete required course, reminders start ${settings.lead_days === 0 ? "on its deadline" : `${settings.lead_days} days before its deadline`} and repeat at most every ${settings.repeat_days} ${settings.repeat_days === 1 ? "day" : "days"} while it remains incomplete.` : "Learner reminders are off."}</p>
        {settings.digest_enabled ? <>
          <p>Administrators receive a summary on {TRAINING_WEEKDAYS[settings.digest_weekday - 1]} when required training is overdue. Follow-up alerts begin at {settings.escalation_days} {settings.escalation_days === 1 ? "day" : "days"} overdue and repeat at most weekly while work remains at or beyond that threshold.</p>
          <p>Audience: {settings.recipient_ids.length ? `${settings.recipient_ids.length} selected ${settings.recipient_ids.length === 1 ? "administrator" : "administrators"}` : "all active administrators authorized for this facility"}.</p>
        </> : <p>Administrator summaries and follow-up alerts are off.</p>}
        <p className="text-xs text-muted-foreground">This is a settings preview, not a delivery confirmation. Account preferences and current facility access apply; course deadlines stay unchanged.</p>
      </div>
...
export function TrainingReportAutomation({ facilityId, filters, onOpen }: { facilityId: string; filters: TrainingEnrollmentFilters; onOpen: (filters: SavedTrainingFilters) => void }) {
  ...
  return <details className="rounded-lg border p-4 space-y-4" onToggle={e => setOpen(e.currentTarget.open)}><summary className="cursor-pointer font-semibold">Reminders & scheduled training reports</summary>
    {query.isError ? <QueryError what="training automation" error={query.error} onRetry={() => void query.refetch()} /> : query.isLoading ? <p>Loading training automation…</p> : query.data && <>
      {canManage ? <><h3 className="font-semibold">Reminder settings</h3><TrainingReminderForm key={JSON.stringify(query.data.settings)} facilityId={facilityId} data={query.data} /></> : <p>Facility administrators manage reminder settings and report subscriptions.</p>}
