...
export function TrainingReminderReceipts({ facilityId, employeeId }: { facilityId: string; employeeId?: string }) {
  ...
  return <details className="border rounded p-3" onToggle={e => setOpen(e.currentTarget.open)}><summary className="cursor-pointer font-medium">Training reminders and delivery results</summary>
    <p className="text-sm my-2">The daily job reminds learners about unfinished required courses due within seven days or already overdue, at most once per course per seven days. Administrators receive a weekly overdue summary. Channel delivery follows account notification preferences. These are the latest 100 reminder receipts.</p>
