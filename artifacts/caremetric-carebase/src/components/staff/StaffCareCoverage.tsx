...
export function StaffCareCoverage({ data }: { data: StaffCareCoverageData }) {
  ...
  return <section className="space-y-3 rounded border p-3">
    ...
    <details><summary className="text-sm">{gaps.length} intervals need adult / awake coverage review</summary><ul className="mt-2 space-y-1 text-xs">{gaps.map(r => <li key={r.starts}>{time(r.starts)} – {time(r.ends)}: {r.adult21_staff} qualified staff aged 21+, {r.awake_staff}/{r.direct_staff} qualified direct-care staff recorded awake, {r.unknown_staff} staff with incomplete qualification or availability evidence.</li>)}</ul></details>
