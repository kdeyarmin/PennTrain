...
function getNavSections(role: AuthUser["role"], showPchAlrModules: boolean): NavSection[] {
  if (role === "platform_admin") {
    ...
    // Lower-frequency configuration surfaces live under Advanced so the primary scan stays action-first.
    return [
      {
        items: [
      ...
      },
      {
        ...
        items: [
          ...
          ...(showPchAlrModules ? [{ href: "/app/state-forms", label: "State forms", icon: ClipboardList }] : []),
          ...(showPchAlrModules ? [{ href: "/app/resident-finance", label: "Resident finance", icon: Landmark }] : []),
          ...(showPchAlrModules ? [{ href: "/app/qapi", label: "QAPI & quality", icon: BarChart3 }] : []),
    ...
    // Advanced collapses lower-frequency documentation tools by default.
    return [
      {
        items: [
          { href: "/app/today", label: "Today", icon: Activity },
