...
export function UnsyncedDraftsPanel() {
  ...
  const runSyncAll = async () => {
    try {
      const [result, observationResult] = await Promise.all([
        syncAll.mutateAsync(),
        syncAllObservations.mutateAsync(),
      ]);
      const applied = result.applied + observationResult.applied;
      const attempted = result.attempted + observationResult.attempted;
      const needsAttention = result.needsReview + result.failed
        + observationResult.needsReview + observationResult.failed;
      if (result.wipeRequired || observationResult.wipeRequired) {
        ...
        toast({
          title: `${applied} recorded, ${needsAttention} need attention`,
