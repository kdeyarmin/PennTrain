...
export function ResidentPortalWorkspace({ residentId }: { residentId: string }) {
  ...
  return (
    <Card>
      <CardHeader><div className="flex flex-wrap items-start justify-between gap-4"><div><CardTitle className="flex items-center gap-2"><ShieldCheck className="h-5 w-5" />Designated-Person Portal</CardTitle><CardDescription>Issue expiring, consent-gated access to selected resident summaries. Every access, request, response, and download is logged.</CardDescription></div><div className="flex gap-2"><Button variant="outline" onClick={() => { setPaymentProvider(""); setPaymentUrl(""); setPaymentAmount(""); setPaymentExpires(defaultExpiry()); setPaymentOpen(true); }}><CreditCard className="mr-2 h-4 w-4" />Payment link</Button><Button onClick={() => { setName(""); setRelationship(""); setEmail(""); setExpiresOn(defaultExpiry()); setPermissions(["schedule", "messages"]); setGeneratedLink(""); setCreateOpen(true); }}><UserRoundPlus className="mr-2 h-4 w-4" />Create access</Button></div></div></CardHeader>
