...
export function ResidentAgreementWorkspace({
  ...
  const [guestSignerRole, setGuestSignerRole] = useState("designated_person");
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-5">
        ...
        {!!query.data?.guestGrants.length && <div><h3 className="mb-2 flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="h-4 w-4" />External access</h3><div className="space-y-2">{query.data.guestGrants.map(grant => <div key={grant.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border p-3 text-sm"><div><p className="font-medium">{grant.guest_label}</p><p className="text-xs text-muted-foreground">{humanize(grant.signer_role)} · {grant.allowed_version_ids.length} agreement(s) · expires {new Date(grant.expires_at).toLocaleString()}</p></div><Badge variant="outline">{grant.revoked_at ? "Revoked" : new Date(grant.expires_at) <= new Date() ? "Expired" : grant.accepted_at ? "Accepted" : "Issued"}</Badge>{canManage && !grant.revoked_at && <Button size="sm" variant="outline" onClick={() => revokeGrant.mutate({ residentId, grantId: grant.id, reason: "Facility revoked external agreement access" }, { onSuccess: () => toast({ title: "External access revoked" }), onError: (error: Error) => toast({ title: "Could not revoke access", description: error.message, variant: "destructive" }) })}>Revoke</Button>}</div>)}</div></div>}
      ...
      <Dialog open={agreementOpen} onOpenChange={setAgreementOpen}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl"><DialogHeader><DialogTitle>{agreementForm.agreementId ? "Publish agreement amendment" : "Publish resident agreement"}</DialogTitle><DialogDescription>The canonical text and optional resident document are hashed into an immutable version.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2"><Label htmlFor={`${__fieldIds}-canonical-agreement-text`}>Canonical agreement text</Label><Textarea id={`${__fieldIds}-canonical-agreement-text`} className="min-h-40" value={agreementForm.contentText} onChange={event => setAgreementForm(current => ({ ...current, contentText: event.target.value }))} /></div>
        <div><Label htmlFor={`${__fieldIds}-effective-date-and-time`}>Effective date and time</Label><Input id={`${__fieldIds}-effective-date-and-time`} type="datetime-local" value={agreementForm.effectiveAt} onChange={event => setAgreementForm(current => ({ ...current, effectiveAt: event.target.value }))} /></div>
      ...
      <Dialog open={responseOpen} onOpenChange={setResponseOpen}><DialogContent className="max-h-[90vh] overflow-y-auto"><DialogHeader><DialogTitle>Record resident agreement response</DialogTitle><DialogDescription>Use this for staff-assisted signing, resident portal authentication, or imported wet-signature documentation.</DialogDescription></DialogHeader><div className="grid gap-3 sm:grid-cols-2">
        ...
        <div><Label htmlFor={`${__fieldIds}-legal-authority`}>Legal authority</Label><Input id={`${__fieldIds}-legal-authority`} value={responseForm.legalAuthority} onChange={event => setResponseForm(current => ({ ...current, legalAuthority: event.target.value }))} /></div>
        <div className="sm:col-span-2"><Label htmlFor={`${__fieldIds}-attestation`}>Attestation</Label><Textarea id={`${__fieldIds}-attestation`} value={responseForm.attestation} onChange={event => setResponseForm(current => ({ ...current, attestation: event.target.value }))} /></div>
      ...
      <Dialog open={shareOpen} onOpenChange={(open) => {
        if (!open) {
          ...
          setGuestVersionIds([]);
          setGuestDays("7");
          setGuestSignerRole("designated_person");
        } else {
          setShareOpen(true);
        }
      }}><DialogContent><DialogHeader><DialogTitle>Create external signing link</DialogTitle><DialogDescription>Select one signer role and exact current versions that require that role. The role is cryptographically bound to the link.</DialogDescription></DialogHeader><div className="space-y-3"><div><Label htmlFor={`${__fieldIds}-guest-label`}>Guest label</Label><Input id={`${__fieldIds}-guest-label`} value={guestLabel} onChange={event => setGuestLabel(event.target.value)} /></div><div><Label htmlFor={`${__fieldIds}-authorized-signer-role`}>Authorized signer role</Label><Select value={guestSignerRole} onValueChange={value => { setGuestSignerRole(value); setGuestVersionIds([]); }}><SelectTrigger id={`${__fieldIds}-authorized-signer-role`}><SelectValue /></SelectTrigger><SelectContent>{SIGNER_ROLES.map(value => <SelectItem key={value} value={value}>{humanize(value)}</SelectItem>)}</SelectContent></Select></div><div><Label id={`${__fieldIds}-agreement-versions`}>Agreement versions</Label><div className="mt-2 space-y-2" role="group" aria-labelledby={`${__fieldIds}-agreement-versions`}>{activeVersions.filter(version => version.required_signer_roles.includes(guestSignerRole)).map(version => { const agreement = query.data?.agreements.find(item => item.id === version.agreement_id); return <label key={version.id} className="flex items-start gap-2 rounded-md border p-2 text-sm"><Checkbox checked={guestVersionIds.includes(version.id)} onCheckedChange={checked => setGuestVersionIds(current => checked === true ? [...current, version.id] : current.filter(id => id !== version.id))} /><span>{agreement?.title}<span className="block text-xs text-muted-foreground">Version {version.version_label} · {humanize(guestSignerRole)}</span></span></label>; })}</div></div><div><Label htmlFor={`${__fieldIds}-expires-in-days`}>Expires in days</Label><Input id={`${__fieldIds}-expires-in-days`} type="number" min={1} max={30} value={guestDays} onChange={event => setGuestDays(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setShareOpen(false)}>Cancel</Button><Button disabled={issueGrant.isPending || !guestLabel.trim() || !guestVersionIds.length || Number(guestDays) < 1} onClick={createLink}>{issueGrant.isPending ? "Creating…" : "Create secure link"}</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={!!copySignature} onOpenChange={open => {
        if (!open) {
          setCopySignature(null);
          setDeliveryMethod("email");
        }
      }}><DialogContent><DialogHeader><DialogTitle>Record copy delivery</DialogTitle><DialogDescription>This can be recorded once and becomes part of the immutable signature documentation.</DialogDescription></DialogHeader><div><Label htmlFor={`${__fieldIds}-delivery-method`}>Delivery method</Label><Select value={deliveryMethod} onValueChange={setDeliveryMethod}><SelectTrigger id={`${__fieldIds}-delivery-method`}><SelectValue /></SelectTrigger><SelectContent>{["email","portal","printed","mail","in_person","other"].map(value => <SelectItem key={value} value={value}>{humanize(value)}</SelectItem>)}</SelectContent></Select></div><DialogFooter><Button variant="outline" onClick={() => { setCopySignature(null); setDeliveryMethod("email"); }}>Cancel</Button><Button disabled={markCopy.isPending} onClick={() => copySignature && markCopy.mutate({ residentId, signatureId: copySignature.id, deliveredAt: new Date().toISOString(), deliveryMethod }, { onSuccess: () => { setCopySignature(null); setDeliveryMethod("email"); toast({ title: "Copy delivery recorded" }); }, onError: (error: Error) => toast({ title: "Could not record copy delivery", description: error.message, variant: "destructive" }) })}>Record delivery now</Button></DialogFooter></DialogContent></Dialog>
    </Card>
  );
}
