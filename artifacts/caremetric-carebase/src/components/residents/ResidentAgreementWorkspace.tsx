...
export function ResidentAgreementWorkspace({
  ...
  return (
    <Card>
      ...
      <Dialog open={shareOpen} onOpenChange={setShareOpen}><DialogContent><DialogHeader><DialogTitle>Create external signing link</DialogTitle><DialogDescription>Select one signer role and exact current versions that require that role. The role is cryptographically bound to the link.</DialogDescription></DialogHeader><div className="space-y-3"><div><Label>Guest label</Label><Input value={guestLabel} onChange={event => setGuestLabel(event.target.value)} /></div><div><Label>Authorized signer role</Label><Select value={guestSignerRole} onValueChange={value => { setGuestSignerRole(value); setGuestVersionIds([]); }}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{SIGNER_ROLES.map(value => <SelectItem key={value} value={value}>{humanize(value)}</SelectItem>)}</SelectContent></Select></div><div><Label>Agreement versions</Label><div className="mt-2 space-y-2">{activeVersions.filter(version => version.required_signer_roles.includes(guestSignerRole)).map(version => { const agreement = query.data?.agreements.find(item => item.id === version.agreement_id); return <label key={version.id} className="flex items-start gap-2 rounded-md border p-2 text-sm"><Checkbox checked={guestVersionIds.includes(version.id)} onCheckedChange={checked => setGuestVersionIds(current => checked === true ? [...current, version.id] : current.filter(id => id !== version.id))} /><span>{agreement?.title}<span className="block text-xs text-muted-foreground">Version {version.version_label} · {humanize(guestSignerRole)}</span></span></label>; })}</div></div><div><Label>Expires in days</Label><Input type="number" min={1} max={30} value={guestDays} onChange={event => setGuestDays(event.target.value)} /></div></div><DialogFooter><Button variant="outline" onClick={() => setShareOpen(false)}>Cancel</Button><Button disabled={issueGrant.isPending || !guestLabel.trim() || !guestVersionIds.length || Number(guestDays) < 1} onClick={createLink}>{issueGrant.isPending ? "Creating…" : "Create secure link"}</Button></DialogFooter></DialogContent></Dialog>

      <Dialog open={!!copySignature} onOpenChange={open => !open && setCopySignature(null)}><DialogContent><DialogHeader><DialogTitle>Record copy delivery</DialogTitle><DialogDescription>This can be recorded once and becomes part of the immutable signature documentation.</DialogDescription></DialogHeader><div><Label>Delivery method</Label><Select value={deliveryMethod} onValueChange={setDeliveryMethod}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent>{["email","portal","printed","mail","in_person","other"].map(value => <SelectItem key={value} value={value}>{humanize(value)}</SelectItem>)}</SelectContent></Select></div><DialogFooter><Button variant="outline" onClick={() => setCopySignature(null)}>Cancel</Button><Button disabled={markCopy.isPending} onClick={() => copySignature && markCopy.mutate({ residentId, signatureId: copySignature.id, deliveredAt: new Date().toISOString(), deliveryMethod }, { onSuccess: () => { setCopySignature(null); toast({ title: "Copy delivery recorded" }); } })}>Record delivery now</Button></DialogFooter></DialogContent></Dialog>
    </Card>
  );
}
