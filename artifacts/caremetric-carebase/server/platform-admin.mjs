  ...
  return async (request) => {
    try {
      ...
      if (operation.operation === "support.identity.resolve") {
        ...
        data = { apiVersion: 1, operations: [...OPERATIONS, ...(config.commandsEnabled && config.packageIngestionEnabled ? ["learning.packages.context", "learning.packages.status", "learning.packages.upload", "learning.packages.accept", "learning.packages.finish"] : []), ...(config.commandsEnabled ? ["commands.preview", "commands.apply", "operations.config.v1"] : []),
