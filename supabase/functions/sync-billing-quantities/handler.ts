...
export type StripeGetFn = (
  path: string,
  secretKey: string,
) => Promise<{ ok: boolean; status: number; data: Record<string, unknown> }>;

export interface SyncBillingQuantitiesDependencies {
  createClient: ClientFactory;
  stripePost: StripePostFn;
  stripeGet: StripeGetFn;
  getEnv?: (name: string) => string | undefined;
  requireCron?: typeof requireCronRequest;
  randomUUID?: () => string;
  nowMs?: () => number;
}
...
export function createSyncBillingQuantitiesHandler({
  ...
  if (stripePriceIds.length > 0) {
    const { data, error } = await admin.from("package_billing_prices")
      .select("stripe_price_id, billing_metric, pricing_model, minimum_quantity, maximum_quantity")
    ...
    prices = ((data ?? []) as PriceRow[]).filter((price) => !!price.stripe_price_id);
  }
  ...
  for (let offset = 0; offset < items.length; offset += concurrency) {
    ...
    await Promise.all(items.slice(offset, offset + concurrency).map(async (item) => {
      ...
      if (price.billing_metric !== "flat" && price.pricing_model !== "flat") {
        ...
        measured = usage ? phase2MeasuredBillingQuantity(price.billing_metric, usage) : null;
      }
      const resolved = resolveSyncTargetQuantity(price, measured);
      if ("error" in resolved) {
        if (resolved.error === "usage_required") {
          failed++;
      ...
      if (requiresProviderVerification) {
        ...
        // the mutation.
        const verification = await stripeGet(
      ...
      if (!providerSucceeded) {
        const stripeResult = await stripePost(
          `/v1/subscription_items/${encodeURIComponent(item.stripe_subscription_item_id)}`,
          stripeSecretKey,
