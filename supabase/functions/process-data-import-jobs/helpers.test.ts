...
Deno.test("durable import domains include only service-role-safe direct-table processors", () => {
...
Deno.test("pending durable domains include all domains whose apply RPC requires auth.uid / authenticated grant", () => {
  assertEquals(Array.from(PENDING_DURABLE_DOMAINS), ["rooms", "credentials", "training_records", "incidents"]);
});
