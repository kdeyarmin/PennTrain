...
export interface PackageZipEntry { name: string; size: number; originalSize: number; compression: number }
export type PackageUnzip = (bytes: Uint8Array, options: { filter: (entry: PackageZipEntry) => boolean }) => Record<string, Uint8Array>;
export function createPackageArchiveReader(unzipSync: PackageUnzip) {
return function readPackageArchive(bytes: Uint8Array, onlyPath?: string): Record<string, Uint8Array> {
