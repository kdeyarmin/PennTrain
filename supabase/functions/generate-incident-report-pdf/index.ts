...
function wrapTextToLines(text: string, maxWidth: number, font: PDFFont, size: number): string[] {
  ...
  // before any draw runs, which 500'd the whole report.
...
class PdfWriter {
  ...
  paragraph(text: string) {
    // Delegate to wrapTextToLines so a single long token (a pasted pharmacy-portal URL in a
    // narrative) is hard-broken instead of drawn past the page edge and lost in print.
