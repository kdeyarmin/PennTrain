...
export function createImpersonateUserHandler({
  createClient,
  ...
  if (action === "start_bound") {
    ...
    if (targetProfile.role === "platform_admin") {
      return json(req, { error: "cannot impersonate another platform_admin" }, 403);
    }
    ...
    if (auditInsertError) {
      return json(req, { error: "Failed to record the required audit log entry; impersonation aborted." }, 500);
    }
  ...
  if (action === "bind" || action === "end") {
    ...
    if (context.target_session_id !== currentSessionId) {
      return json(req, { error: "Current Auth session is not the bounded impersonation session" }, 403);
    }
    const { error: auditInsertError } = await adminClient.from("audit_logs").insert({
    ...
    if (auditInsertError) {
      return json(req, { error: "Failed to record the impersonation-end audit log entry." }, 500);
    }
