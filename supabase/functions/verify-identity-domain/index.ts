...
function json(request: Request, body: unknown, status = 200, requestId?: string): Response {
  return new Response(JSON.stringify(body), {
    status,
    headers: {
      "Content-Type": "application/json",
      "Cache-Control": "no-store",
      ...corsHeadersForRequest(request, { headers: "authorization, x-client-info, apikey, content-type, x-request-id, x-correlation-id" }),
...
Deno.serve(async (request: Request) => {
  if (request.method === "OPTIONS") return corsPreflightResponse(request, { headers: "authorization, x-client-info, apikey, content-type, x-request-id, x-correlation-id" });
