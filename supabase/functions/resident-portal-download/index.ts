...
function json(req: Request, body: unknown, status = 200) {
  return new Response(JSON.stringify(body), { status, headers: { "Content-Type": "application/json", ...corsHeadersForRequest(req, { methods: "POST, OPTIONS" }) } });
}
...
Deno.serve(async (req: Request) => {
  if (req.method === "OPTIONS") return corsPreflightResponse(req, { methods: "POST, OPTIONS" });
  if (req.method !== "POST") return json(req, { error: "Method not allowed" }, 405);
  const supabaseUrl = Deno.env.get("SUPABASE_URL");
  const serviceRoleKey = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY");
  if (!supabaseUrl || !serviceRoleKey) return json(req, { error: "Service is not configured" }, 500);
  ...
  if (authorizeError) {
    ...
    return json(req, { authorized: false, error: "Authorization failed" }, 403);
  }
  if (!decision?.authorized || !decision.bucket || !decision.path) return json(req, { authorized: false, error: "Access denied" }, 403);
  ...
  if (error || !data?.signedUrl) {
    ...
    return json(req, { error: "Download link could not be created" }, 500);
  }
  return json(req, { authorized: true, url: data.signedUrl, fileName: decision.fileName, expiresIn: SIGNED_URL_TTL_SECONDS });
});
