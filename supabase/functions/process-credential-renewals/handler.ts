...
export function createProcessCredentialRenewalsHandler({
  createClient,
  getEnv = (name: string) => Deno.env.get(name),
  authorizeCron = requireCronRequest,
}: {
  createClient: typeof SupabaseCreateClient;
  getEnv?: (name: string) => string | undefined;
  authorizeCron?: typeof requireCronRequest;
}) {
  ...
  if (req.headers.has(CRON_SECRET_HEADER)) {
    const denied = authorizeCron(req, corsHeadersForRequest(req));
  ...
  for (const sub of submissions) {
    try {
      ...
      const baa = getEnv("ANTHROPIC_BAA_CONFIRMED") === "true";
      ...
      if (baa && apiKey) {
        try {
          ...
          const model = getEnv("ANTHROPIC_DOCUMENT_ANALYZER_MODEL")
            || getEnv("ANTHROPIC_CREDENTIAL_RENEWAL_MODEL")
