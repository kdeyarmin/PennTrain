...
Deno.serve(async (req: Request) => {
  ...
  if (!jobId) {
    const { data, error } = await callerClient.rpc("start_data_import_job", {
      p_domain: DOMAIN, p_file_name: (typeof body.file_name === "string" ? body.file_name : "incidents.csv").slice(0, 255),
  ...
  for (let index = offset; index < endIndex; index++) {
    ...
    if (!rowErrors.length && externalId) {
      ...
      if (found.error) return await abortRun(`Row ${rowNumber}: resident lookup failed: ${found.error.message}`);
      let data = found.data;
      if (!data) {
