// @ts-nocheck -- HeyGen job state; CORS hardened
...
Deno.serve(async (req: Request) => {
  ...
  if (!supabaseUrl || !anonKey || !serviceRoleKey) return json(req, { error: "Service is not configured" }, 503);
  ...
  if (!WRITER_ROLES.includes(callerProfile.role as string)) {
    return json(req, { error: "not authorized to check course video status" }, 403);
  }
  ...
  if (result.status === "completed") {
    return result.video_url
      ...
      : json(req, { success: true, status: "completed" });
  }
  if (result.status === "failed") {
    return json(req, { success: true, status: "failed", error: result.error });
  }
  return json(req, { success: true, status: result.status });
});
