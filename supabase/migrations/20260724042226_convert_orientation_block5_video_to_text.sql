...
-- (applied 2026-07-24 as version 20260724042226 but never committed to git).
...
-- ships the END state of the whole 042226 -> 044044 video saga (block 5 is a
-- video carrying the final trimmed HeyGen render b4bc8986...), so re-enacting
-- this intermediate step against the seed would break the publish gate's
-- 180-minute invariant at 20260724042243 (observed in CI: 177 != 180). The
-- conversion below therefore applies only when block 5 still carries a
-- pre-trimmed-render state -- production's actual history -- and no-ops on
-- fresh replays, which already sit at the final state.
--
...
begin
  if (
    select coalesce(body->'heygen'->>'video_id', '')
    from public.course_blocks
    where id = 'd8424426-4992-4d3a-aa53-b8017480a30f'::uuid
  ) = 'b4bc89866d5e4a7087165f31ca6faba3' then
    -- Final trimmed render already present (fresh replay of the end-state seed).
    return;
  end if;
