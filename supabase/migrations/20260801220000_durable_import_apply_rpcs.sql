-- D3 durable import: service_role-only SECURITY DEFINER apply RPCs.
-- Does NOT widen table grants and does NOT weaken interactive user RPCs.
-- Actor fields (verified_by / reported_by) are NULL — system-applied import.
...
begin
  if coalesce(auth.role(), '') is distinct from 'service_role' then
    raise exception 'import_apply_training_record is service_role only' using errcode = '42501';
  end if;
  ...
  if p_record_id is not null then
    ...
    if v_existing.organization_id is distinct from p_organization_id then
      raise exception 'training record is outside the job organization' using errcode = '23514';
    end if;
  ...
  if v_employee.organization_id is distinct from p_organization_id then
    raise exception 'employee is outside the job organization' using errcode = '23514';
  end if;
  if not exists (
  ...
  -- System-applied import: no human reviewer identity.
...
grant execute on function public.import_apply_training_record(uuid, uuid, jsonb) to service_role;
...
begin
  if coalesce(auth.role(), '') is distinct from 'service_role' then
    raise exception 'import_apply_employee_credential is service_role only' using errcode = '42501';
  end if;
  if p_organization_id is null then
    raise exception 'organization_id is required' using errcode = '22023';
  end if;
  if jsonb_typeof(coalesce(p_payload, '{}'::jsonb)) <> 'object' then
    raise exception 'A credential payload is required' using errcode = '22023';
  end if;
  ...
  if p_credential_id is not null then
    ...
    if v_existing.organization_id is distinct from p_organization_id then
      raise exception 'credential is outside the job organization' using errcode = '23514';
    end if;
...
from public, anon, authenticated;
grant execute on function public.import_apply_employee_credential(uuid, uuid, jsonb) to service_role;
...
begin
  if coalesce(auth.role(), '') is distinct from 'service_role' then
    raise exception 'import_apply_room_with_beds is service_role only' using errcode = '42501';
  end if;
  if p_organization_id is null then
    raise exception 'organization_id is required' using errcode = '22023';
  end if;
  ...
  if v_facility.organization_id is distinct from p_organization_id then
    raise exception 'facility is outside the job organization' using errcode = '23514';
  end if;
...
revoke all on function public.import_apply_room_with_beds(uuid, uuid, text, text, text, text, integer, text, integer)
from public, anon, authenticated;
grant execute on function public.import_apply_room_with_beds(uuid, uuid, text, text, text, text, integer, text, integer) to service_role;
...
begin
  if coalesce(auth.role(), '') is distinct from 'service_role' then
    raise exception 'import_apply_incident is service_role only' using errcode = '42501';
  end if;
  if p_organization_id is null then
    raise exception 'organization_id is required' using errcode = '22023';
  end if;
  ...
  if p_resident_id is not null and not exists (
    ...
    raise exception 'resident is outside the incident facility' using errcode = '23514';
  end if;
  ...
  insert into public.incidents(
    ...
    p_organization_id, p_facility_id, p_incident_type, p_occurred_at, null,
...
revoke all on function public.import_apply_incident(
  uuid, uuid, text, timestamptz, uuid, text, text, text, text, text
) from public, anon, authenticated;
grant execute on function public.import_apply_incident(
  uuid, uuid, text, timestamptz, uuid, text, text, text, text, text
) to service_role;
