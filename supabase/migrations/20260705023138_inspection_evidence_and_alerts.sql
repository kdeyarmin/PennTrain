...
alter table public.training_documents drop constraint training_documents_document_type_check;
alter table public.training_documents add constraint training_documents_document_type_check
  check (document_type in (
    'certificate','roster','practicum_form','transcript','external_certificate',
    'competency_attachment','inspection_report','other'));

alter table public.alerts add column inspection_item_id uuid references public.inspection_items(id);
create index alerts_inspection_item_idx on public.alerts(inspection_item_id);
...
begin
  ...
  set
    ...
    next_due_date = coalesce(h.last_date, i.install_date, i.created_at::date) + i.inspection_interval_days
  from (
  ...
  set status = case
    ...
    when i.next_due_date < current_date then 'expired'
    when i.next_due_date <= current_date + 30 then 'due_soon'
    else 'compliant'
  end
  where i.is_active;
