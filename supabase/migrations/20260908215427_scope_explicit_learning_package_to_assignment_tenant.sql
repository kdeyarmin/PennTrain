-- Explicit runtime package selection must obey the same tenant boundary as automatic selection.
-- The function is SECURITY DEFINER: a foreign package UUID on the same course version otherwise
-- bypasses learner_packages_select and creates a runtime session for content outside the tenant.
-- Keep shared packages (organization_id IS NULL), owned packages, resume sequences and grants.
