-- A throttle that survives its own denial (Codex review of PR #484).
--
-- 20260905230000 added a rate limit and a failed-token log to the guest surfaces. Neither ever
-- recorded anything, and the reproduction is two lines:
--
--   select public.get_move_in_guest_workspace('definitely-not-a-real-token');
--   ERROR:  Move-in guest access denied
--   -> guest_request_windows: 0 rows.  guest_token_failures: 0 rows.
--
-- PostgREST runs the whole RPC in ONE transaction, and the 42501 aborted it -- taking with it the
-- counter row and the failure-log row the gate had just written. So the throttle counted only the
-- requests that SUCCEEDED: a real guest making sixty calls a minute was limited, and a scanner
-- trying ten thousand tokens was not, while GuestAccessHealthCard showed zero for ever. The
-- control was inert against precisely the thing it was built to detect.
--
-- There is no fix inside one transaction: Postgres cannot commit inside a function. For the write
-- to survive, the denial must not raise. It does not raise any more.
--
-- What replaces it, and why the client does not change: a denied call sets
-- `response.status = 403` and RETURNS a body shaped like a PostgREST error
-- ({code, message, details, hint}). PostgREST answers 403, the transaction COMMITS, and
-- supabase-js hands the caller an `error` indistinguishable from the one it got before. Measured
-- against the local stack, including the failure mode this avoids: a 403 whose body is a bare
-- `false` or `null` reaches postgrest-js as a FALSY error with null data -- a silent nothing, which
-- is worse than the bug. That is why the eight boolean/uuid RPCs return jsonb now; a scalar body
-- cannot carry a denial the client will notice.
--
--   __probe_bool  -> error=false  truthy=false   <- silent failure
--   __probe_uuid  -> error=null   truthy=false   <- silent failure
--   __probe_obj   -> error={code,message,...}    truthy=true    <- what we ship
--
-- assert_guest_request_allowed is DROPPED rather than kept as a raising wrapper. A function whose
-- contract is "abort the transaction" cannot be used by anything PostgREST calls without
-- reintroducing exactly this defect, and leaving it there is an invitation. Its replacement is a
-- verdict -- null when allowed, a message when not -- and it needs no anon grant at all, because
-- every caller is SECURITY DEFINER. The anon-executable surface goes from 21 to 20.
--
-- BACKLOG.md I29.

------------------------------------------------------------------------------------------------
-- 1. Denial, expressed as a value rather than an abort
------------------------------------------------------------------------------------------------
create or replace function app_private.guest_denied(p_message text)
returns jsonb
language plpgsql
set search_path = ''
as $$
begin
  -- 403 with a PostgREST-shaped body: the caller sees the same error it always saw, and the
  -- transaction commits, so whatever the gate recorded on the way in is still there afterwards.
  perform pg_catalog.set_config('response.status', '403', true);
  return jsonb_build_object('code', '42501', 'message', p_message, 'details', null, 'hint', null);
end;
$$;

revoke all on function app_private.guest_denied(text) from public, anon, authenticated, service_role;

comment on function app_private.guest_denied(text) is
  'A guest denial as a returned value: sets response.status 403 and returns a PostgREST-shaped error body, so the transaction commits and the throttle counters written before it survive.';

------------------------------------------------------------------------------------------------
-- 2. The gate, answering instead of aborting
------------------------------------------------------------------------------------------------
drop function if exists public.assert_guest_request_allowed(text, text);

CREATE OR REPLACE FUNCTION public.guest_request_denial(p_surface text, p_token text)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
begin
  ...
  -- RETURNED, not raised. Every one of these decisions comes AFTER the counter write above, and
  -- PostgREST runs the whole call in one transaction -- so raising here rolled back the very row
  -- that records the attempt. The throttle counted only requests that SUCCEEDED, which is the
  -- opposite of a throttle. The caller turns a non-null answer into a 403 without aborting.
  if v_unknown > c_unknown_limit then
    return 'Too many invalid access attempts from this connection. Wait a minute and open the link again.';
  end if;
  if v_count > c_request_limit then
    return 'Too many requests from this connection. Wait a minute and try again.';
  end if;
  ...
  if v_org is not null and exists (
    ...
    return 'This facility''s account is not active. Please contact the facility directly.';
  end if;
...
comment on function public.guest_request_denial(text, text) is
  'Records a guest request and returns why it is denied, or null when it is allowed. Never raises for a denial -- see 20260905360000. Needs no anon grant: every caller is SECURITY DEFINER.';

------------------------------------------------------------------------------------------------
-- 3. The seventeen guest RPCs. Eight return jsonb now; see the header for why a scalar cannot
--    carry a denial the client will notice.
------------------------------------------------------------------------------------------------
drop function if exists public.accept_move_in_guest_terms(p_token text, p_fingerprint text);
drop function if exists public.accept_resident_agreement_guest_terms(p_token text, p_device_evidence text);
drop function if exists public.accept_resident_portal_terms(p_token text, p_terms_version text, p_request_fingerprint_sha256 text);
drop function if exists public.post_resident_portal_message(p_token text, p_body text, p_request_fingerprint_sha256 text);
drop function if exists public.post_resident_portal_request(p_token text, p_request_type text, p_subject text, p_detail text, p_request_fingerprint_sha256 text);
drop function if exists public.respond_resident_portal_schedule_event(p_token text, p_calendar_event_id uuid, p_response text, p_note text);
drop function if exists public.respond_to_resident_agreement_guest(p_token text, p_version_id uuid, p_outcome text, p_signer_name text, p_signer_role text, p_relationship text, p_legal_authority text, p_attestation text, p_reason text, p_witness_name text, p_witness_relationship text, p_device_evidence text, p_ip_evidence text);
drop function if exists public.sign_move_in_guest_task(p_token text, p_task_id uuid, p_signer_name text, p_relationship text, p_attestation text);
...
begin
  ...
  v_guest_denial := public.guest_request_denial('evidence_guest', p_token);
...
AS $function$
declare
  v_guest_denial text; v public.move_in_guest_grants%rowtype;
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  v_guest_denial := public.guest_request_denial('move_in_guest', p_token);
  if v_guest_denial is not null then return app_private.guest_denied(v_guest_denial); end if;
  ...
  if not found or v.revoked_at is not null or v.expires_at <= now() then
    return app_private.guest_denied('Move-in guest link is invalid or expired');
  end if;
...
AS $function$
declare
  v_guest_denial text; v public.resident_agreement_guest_grants%rowtype; v_hash text;
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  v_guest_denial := public.guest_request_denial('resident_agreement_guest', p_token);
  if v_guest_denial is not null then return app_private.guest_denied(v_guest_denial); end if;
  ...
  if not found or v.revoked_at is not null or v.expires_at <= now() then
    return app_private.guest_denied('Resident agreement link is invalid or expired');
  end if;
...
AS $function$
declare
  v_guest_denial text; v_grant public.resident_portal_grants%rowtype;
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  ...
  -- to_jsonb, not a bare `false`: plpgsql casts a boolean to jsonb through its TEXT output,
  -- which is `f` and is not valid JSON, so the refusal path raised 22P02 instead of refusing.
  if v_grant.id is null or v_grant.terms_version <> p_terms_version then return to_jsonb(false); end if;
...
begin
  ...
  if v_grant.id is null or v_grant.accepted_terms_at is null or not ('documents' = any(v_grant.permissions))
    ...
    return app_private.guest_denied('Portal document access denied');
  end if;
...
begin
  ...
  where token_sha256 = encode(extensions.digest(convert_to(p_token, 'utf8'), 'sha256'), 'hex');
  if not found or v.revoked_at is not null or v.expires_at <= now() then
    return app_private.guest_denied('Move-in guest access denied');
  end if;
  if v.accepted_at is null then
    return app_private.guest_denied('Move-in guest terms acceptance required');
  end if;
...
begin
  ...
  where token_sha256 = encode(extensions.digest(convert_to(p_token, 'utf8'), 'sha256'), 'hex');
  if not found or v.revoked_at is not null or v.expires_at <= now() then
    return app_private.guest_denied('Resident agreement access denied');
  end if;
  if v.accepted_at is null then
    return app_private.guest_denied('Resident agreement terms acceptance required');
  end if;
  select * into v_resident from public.residents where id = v.resident_id;
  insert into public.resident_agreement_guest_access_events(
...
AS $function$
declare
  v_guest_denial text; v_grant public.resident_portal_grants%rowtype; v_snapshot jsonb; v_requests jsonb := '[]'::jsonb; v_payment jsonb := 'null'::jsonb;
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  v_guest_denial := public.guest_request_denial('resident_portal', p_token);
  if v_guest_denial is not null then return app_private.guest_denied(v_guest_denial); end if;
...
begin
  ...
  if v_grant.id is null or v_grant.accepted_terms_at is null
     or not ('messages' = any(v_grant.permissions)) then return to_jsonb(false); end if;
...
AS $function$
declare
  v_guest_denial text; v_grant public.resident_portal_grants%rowtype; v_id uuid;
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  v_guest_denial := public.guest_request_denial('resident_portal', p_token);
  if v_guest_denial is not null then return app_private.guest_denied(v_guest_denial); end if;
  ...
  if v_grant.id is null or v_grant.accepted_terms_at is null or not ('requests' = any(v_grant.permissions)) then
    return app_private.guest_denied('Portal request access denied');
  end if;
  if p_request_fingerprint_sha256 is not null and p_request_fingerprint_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception 'Portal request fingerprint is invalid' using errcode = '22023';
  end if;
...
begin
  -- Throttle, record and scope-check this guest request before anything else runs.
  v_guest_denial := public.guest_request_denial('safety_report', p_token);
  if v_guest_denial is not null then return app_private.guest_denied(v_guest_denial); end if;
...
begin
  ...
  if v_grant.id is null or v_grant.accepted_terms_at is null or not ('schedule' = any(v_grant.permissions))
    ...
    return app_private.guest_denied('Schedule response is outside portal scope');
  end if;
...
begin
  ...
  then
    return app_private.guest_denied('Resident agreement signing denied');
  end if;
...
begin
  ...
  if not found or v.revoked_at is not null or v.expires_at <= now() or v.accepted_at is null
    ...
    return app_private.guest_denied('Move-in guest signing denied');
  end if;
...
$function$
;
grant execute on function public.accept_move_in_guest_terms(p_token text, p_fingerprint text) to anon, authenticated;
grant execute on function public.accept_resident_agreement_guest_terms(p_token text, p_device_evidence text) to anon, authenticated;
grant execute on function public.accept_resident_portal_terms(p_token text, p_terms_version text, p_request_fingerprint_sha256 text) to anon, authenticated;
grant execute on function public.post_resident_portal_message(p_token text, p_body text, p_request_fingerprint_sha256 text) to anon, authenticated;
grant execute on function public.post_resident_portal_request(p_token text, p_request_type text, p_subject text, p_detail text, p_request_fingerprint_sha256 text) to anon, authenticated, service_role;
grant execute on function public.respond_resident_portal_schedule_event(p_token text, p_calendar_event_id uuid, p_response text, p_note text) to anon, authenticated, service_role;
grant execute on function public.respond_to_resident_agreement_guest(p_token text, p_version_id uuid, p_outcome text, p_signer_name text, p_signer_role text, p_relationship text, p_legal_authority text, p_attestation text, p_reason text, p_witness_name text, p_witness_relationship text, p_device_evidence text, p_ip_evidence text) to anon, authenticated;
grant execute on function public.sign_move_in_guest_task(p_token text, p_task_id uuid, p_signer_name text, p_relationship text, p_attestation text) to anon, authenticated;
