...
begin
  ...
  if p_status is null or p_status not in ('all','assigned','in_progress','completed','overdue','paused','canceled')
    ...
    or p_limit is null or p_limit not between 1 and 500 or p_offset is null or p_offset < 0
  ...
  with enrollment as materialized (
    select a.id, a.employee_id, e.first_name || ' ' || e.last_name as student,
      a.facility_id, f.name as facility, a.course_id, coalesce(c.title,'Course unavailable') as course,
    ...
    where a.organization_id=p_organization_id
      ...
      and (btrim(coalesce(p_course_search,''))='' or strpos(lower(coalesce(c.title,'')),lower(btrim(p_course_search)))>0)
  ...
  select jsonb_build_object(
    ...
    -- An export spanning requests fails closed if any report row changes between pages.
    'revision',md5(coalesce(string_agg(md5(to_jsonb(filtered)::text),'' order by id),'')),
    'rows',coalesce((select jsonb_agg(to_jsonb(page) order by assigned_at desc,id) from page),'[]'::jsonb)
...
comment on function public.get_training_enrollment_report(uuid,uuid,text,text,text,date,date,integer,integer) is
  'RLS-scoped enrollment, completion, progress and issued-certificate report. Totals cover the entire filtered set, not the current page. Dates use Pennsylvania calendar days. Canceled enrollments are excluded from the completion denominator.';
