-- Publish Emergency Preparedness Beyond Fire v2 (the video-led rebuild) and move the
-- course's current_version_id onto it.
--
-- 20260726090000 seeded v2 as a draft because its ten video blocks were still
...
-- failure. It just means emergency preparedness continues to serve v1, which is
...
do $publish$
declare
  v_course_id constant uuid := '29c5d1d2-dc44-56d4-8511-a274750db906'::uuid;
  v_version_id constant uuid := '0515864b-7d3b-5990-b991-48a472ecd057'::uuid;
  v_total integer;
  v_pending integer;
begin
  select
  ...
  if v_total = 0 then
    raise notice 'Emergency preparedness v2 not found (or carries no video blocks); nothing to publish.';
    return;
  end if;

  if v_pending > 0 then
    raise notice
      'Emergency preparedness v2 left in draft: % of % video blocks have no re-hosted URL yet. The course stays on v1.',
      v_pending, v_total;
    return;
  end if;
  ...
  raise notice 'Emergency preparedness v2 published across % video blocks; course now serves v2.', v_total;
end;
$publish$;
...
begin
  select c.status, cv.version_number, cv.status
   ...
   where c.id = '29c5d1d2-dc44-56d4-8511-a274750db906'::uuid;

  if not found then
    raise exception 'Emergency preparedness course is missing or has no current version.';
  end if;

  if v_course_status <> 'published' or v_version_status <> 'published' then
    raise exception
      'Emergency preparedness must serve a published version: course is %, current version (v%) is %',
      v_course_status, v_version_number, v_version_status;
  end if;

  select count(*) into v_credits
    from public.course_compliance_credits cc
   where cc.course_id = '29c5d1d2-dc44-56d4-8511-a274750db906'::uuid;

  if v_credits <> 0 then
    raise exception
      'Emergency preparedness now carries % compliance credit row(s); the publish path must move the active mapping onto the current version (see 20260726060000)',
      v_credits;
  end if;

  raise notice 'Emergency preparedness serving v% (published).', v_version_number;
end;
$verify$;
