...
--
-- Strategy: do NOT replace commit_learning_runtime_state (historically breaks phase4_governed_learning pgTAP).
-- Instead, fire an AFTER UPDATE trigger when learning_runtime_sessions.state transitions to 'completed'.
-- The original function already sets state='completed' when completionStatus='completed'.
--
...
comment on function public.bridge_learning_runtime_completion(uuid) is
  'B4 internal: On SCORM/xAPI runtime completion, complete the assignment and upsert employee_training_records / hour buckets when the course maps to a training_type. Not granted to clients; called only from the session-completion trigger.';

-- Fire only on the active → completed transition (the original commit function already performs this update).
...
begin
  ...
  then
    begin
      perform public.bridge_learning_runtime_completion(NEW.id);
    exception
      when others then
        raise warning 'bridge_learning_runtime_completion failed for %: %', NEW.id, SQLERRM;
    end;
  end if;
  return NEW;
end;
...
drop trigger if exists trg_bridge_learning_runtime_completion on public.learning_runtime_sessions;
create trigger trg_bridge_learning_runtime_completion
  after update of state on public.learning_runtime_sessions
  for each row
...
comment on function public.trg_bridge_learning_runtime_completion() is
  'B4: AFTER UPDATE OF state on learning_runtime_sessions — bridges completion into assignment + training records when state first becomes completed.';
