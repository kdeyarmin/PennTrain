-- Adds a resident_documents branch to log_document_access(), mirroring violation_documents'
-- (no employee owner, same org_admin/auditor/assigned-facility_manager authorization shape).
...
begin
  ...
  elsif p_document_table = 'violation_documents' then
    select organization_id, facility_id
      into v_org_id, v_facility_id
    from public.violation_documents where id = p_document_id;
  ...
  elsif p_document_table = 'resident_documents' then
    select organization_id, facility_id
      into v_org_id, v_facility_id
    from public.resident_documents where id = p_document_id;

    if v_org_id is null then
      raise exception 'document not found';
    end if;

    -- Mirrors resident_documents_select in this same migration batch's RLS file -- keep in sync.
    v_authorized := public.is_platform_admin()
      or (v_org_id = (select public.current_org_id())
          and ((select public.current_role()) in ('org_admin','auditor')
               or ((select public.current_role()) = 'facility_manager' and public.is_assigned_to_facility(v_facility_id))));
