-- Single round-trip platform-health snapshot for the admin dashboard, replacing the previously
-- decorative "system health" copy with real counts.
...
begin
  ...
  select jsonb_build_object(
    ...
    'aiGenerationsPending', (select count(*) from public.course_ai_generations where status = 'pending' and created_at > now() - interval '30 days'),
    'aiGenerationsFailed', (select count(*) from public.course_ai_generations where status = 'failed' and created_at > now() - interval '30 days'),
