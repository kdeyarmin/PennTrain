-- Revised drafts inherit the prior version's content wholesale (including its
-- assessmentInfo.assessmentReason/supportPlanReason from the OLD cycle) -- but the reason for
-- *this* new draft (e.g. 'annual' vs 'significant_change') should be reflected in those fields too,
-- so the rendered PDF's "Reason for Assessment" doesn't silently show a stale prior-cycle reason.
