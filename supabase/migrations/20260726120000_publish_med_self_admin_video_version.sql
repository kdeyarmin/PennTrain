-- Publish Medication Self-Administration Support v2 (the video-led rebuild) and move the
-- course's current_version_id onto it.
--
-- 20260726110000 seeded v2 as a draft because its ten video blocks were still
...
-- failure. It just means medication self-administration continues to serve v1, which is
...
do $publish$
declare
  v_course_id constant uuid := '704324fe-a160-5397-9157-7c6d1e1d1e6f'::uuid;
  v_version_id constant uuid := '3c06e92e-153b-5fee-96db-a5212f23286b'::uuid;
  v_total integer;
  v_pending integer;
begin
  select
  ...
  if v_total = 0 then
    raise notice 'Medication self-administration v2 not found (or carries no video blocks); nothing to publish.';
    return;
  end if;

  if v_pending > 0 then
    raise notice
      'Medication self-administration v2 left in draft: % of % video blocks have no re-hosted URL yet. The course stays on v1.',
      v_pending, v_total;
    return;
  end if;
  ...
  raise notice 'Medication self-administration v2 published across % video blocks; course now serves v2.', v_total;
end;
$publish$;
...
begin
  select c.status, cv.version_number, cv.status
   ...
   where c.id = '704324fe-a160-5397-9157-7c6d1e1d1e6f'::uuid;

  if not found then
    raise exception 'Medication self-administration course is missing or has no current version.';
  end if;

  if v_course_status <> 'published' or v_version_status <> 'published' then
    raise exception
      'Medication self-administration must serve a published version: course is %, current version (v%) is %',
      v_course_status, v_version_number, v_version_status;
  end if;

  select count(*) into v_credits
    from public.course_compliance_credits cc
   where cc.course_id = '704324fe-a160-5397-9157-7c6d1e1d1e6f'::uuid;

  if v_credits <> 0 then
    raise exception
      'Medication self-administration now carries % compliance credit row(s); the publish path must move the active mapping onto the current version (see 20260726060000)',
      v_credits;
  end if;

  raise notice 'Medication self-administration serving v% (published).', v_version_number;
end;
$verify$;
