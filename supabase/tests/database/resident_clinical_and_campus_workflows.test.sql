...
select is((select due_date from public.resident_compliance_items where resident_id=pg_temp.id(102) and item_type='annual_medical_evaluation' and completed_date is null),coalesce((select due_date from public.resident_compliance_items where resident_id=pg_temp.id(101) and item_type='annual_medical_evaluation' and completed_date is null order by due_date,id limit 1),((public.pa_today()-120)+interval '1 year')::date),'campus transfer retains the source DME annual cycle');
