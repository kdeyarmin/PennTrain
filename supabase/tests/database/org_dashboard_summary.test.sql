...
-- Server-side org dashboard summary: aggregation parity with the retired client-side
...
select is(
  (public.get_org_dashboard_summary()->'compliance'->>'compliantCount')::int, 2,
  'compliant requirements count training records and practicums together'
);
select is(
...
select is(
  (public.get_org_dashboard_summary()->'compliance'->>'expiredCount')::int, 1,
  'expired requirements are counted exactly'
);
select is(
  (public.get_org_dashboard_summary()->'compliance'->>'missingCount')::int,
  (
    (select count(*)::int from public.employee_training_records where status = 'missing')
    + (select count(*)::int from public.practicums where status = 'missing')
  ),
  'missing count matches the caller-visible missing requirements'
);
select is(
...
select is(
  ...
  'per-facility scores divide compliant by relevant tracked requirements'
);
select is(
  (
    select x->>'complianceScore'
