...
select throws_ok($$insert into public.resident_regulatory_actions(organization_id,facility_id,resident_id,action_type,recipient_role,anchor_at,reason,details) values(pg_temp.id(1),pg_temp.id(11),pg_temp.id(201),'alf_exception_request','department',now(),'Direct clinical write','{"condition":"Continuous skilled nursing need","request_evidence":"Written DHS request","supporting_evidence":"Staff skills and care plan evidence","certifier":"Qualified signed certification","request_decision":"submitted"}')$$,'42501',null,'otherwise valid clinical table writes cannot bypass the manager command');
