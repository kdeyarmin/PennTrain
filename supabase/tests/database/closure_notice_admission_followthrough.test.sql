...
select ok((select bool_and(source_event_id=pg_temp.id(301) and anchor_at=now()+interval '90 days' and due_at=((now() at time zone 'America/New_York')+interval '60 days') at time zone 'America/New_York') from public.resident_regulatory_actions where resident_id=pg_temp.id(201)),'notice deadline keeps exact closure plan anchor instead of admission or creation time');
