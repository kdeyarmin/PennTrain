...
**Last verified against main:** `23a5354` (2026-08-01) — D3 residual safe domains (resident_contacts / training_records / assessments durable; incidents pending)
...
Closed this pass: **Durable import worker expanded to 7 of 8 domains.** `process-data-import-jobs`
now applies `employees`, `rooms`, `credentials`, `residents`, `resident_contacts`,
`training_records`, and `assessments` from `data_import_rows` under service-role scope
validation. `incidents` remains pending because `create_incident_atomic` is security
invoker and requires `auth.uid()`; those jobs still release claims to `ready`.
...
5. Durable import worker — 7 of 8 domains apply from ledger; incidents still pending (`create_incident_atomic` auth.uid())
...
| D3 | Durable import worker (apply from ledger, resume after browser close) | M | in_progress | Durable apply now runs for `employees`, `rooms`, `credentials`, `residents`, `resident_contacts`, `training_records`, and `assessments` from `data_import_rows` (`normalized_row` / `proposed_action`) under service-role org scope. Only `incidents` still released to `ready` (create_incident_atomic requires auth.uid()). |
...
<!-- Register verified with D3 residual safe domains on enhancement/d3-remaining-3-safe-domains-20260801 -->
