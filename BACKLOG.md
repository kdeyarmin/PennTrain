...
| F5 | Reduce total JavaScript shipped to the browser | M | open | **Raised 2026-08-03 when the budget tripwire was moved 4700 -> 5200 KiB.** Raising the tripwire is the response that warning is designed to prompt, but it is not an answer to the number: **4.2 MiB of JavaScript** is a lot to push to a tablet on facility wifi, which is the actual delivery environment. No single route is the culprit -- every audited route chunk is comfortably inside its own budget and the largest is 61.9 KiB -- so this is aggregate feature count plus per-chunk overhead, and the win is in shared/vendor weight rather than in any one page. Worth measuring before cutting: identify the largest shared dependencies, check what is eagerly imported that could be lazy, and confirm whether anything ships to roles that never use it. Not a blocker today (the check warns rather than fails, and every route budget is green), which is exactly why it needs a row instead of a comment |
