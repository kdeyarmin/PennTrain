...
**Last verified against main:** `2951ab8` (2026-08-01) — B4 SCORM→training-record bridge (20260801150000) on post-#377 main
...
Closed this pass: **B4 SCORM/xAPI completion → training record / hour bucket.**
`commit_learning_runtime_state` on first transition to completed calls internal
`bridge_learning_runtime_completion` (quiz blocks still gate; `pa_today()` for dates;
privileged_write for assignment flip; upsert `employee_training_records` + `recalculate_compliance_core` when `courses.training_type_id` is set).
...
| B4 | Bridge SCORM complete → training record / hour bucket | M | done | `commit_learning_runtime_state` → `bridge_learning_runtime_completion` on first completed commit; assignment + training_records + hour buckets when `courses.training_type_id` set; quiz blocks still gate; `pa_today()`; internal-only bridge |
...
<!-- Register verified with B4 migration 20260801150000 on enhancement/b4-scorm-training-record-bridge-v2 -->
