...
**Last verified against main:** `d60dd76` (2026-08-03) — closed item 8: the two offline receipt
ledgers are unified into `offline_draft_receipts`, all four draft kinds now on one table, verified
against a real local Postgres instance (pgTAP, lint, advisors, regenerated types all clean). Earlier —
...
Closed this pass: **The two offline receipt ledgers are one table again.** `20260803140000` renamed
`offline_service_draft_receipts` to `offline_draft_receipts` and widened it to a fourth `draft_kind`
(`clinical_observation`), folding in every row from `offline_observation_draft_receipts` and dropping
that table — the third option this row settled on, executed once Tier 3 made "two draft_kinds on one
table, one draft_kind on a second" the actual state to fix. Larger than the row's own note anticipated:
`sync_offline_change_observation_draft` (added after this row was scoped, by Tier 3) needed the same
table-name reproduction as the two functions the row named, and `sync_offline_clinical_observation_draft`
needed a genuine rewrite rather than a substitution, since it moves from its own table to a fourth kind
on this one. A `DO` block refuses the merge outright if any `(device_id, idempotency_key)` collision
exists between the two source tables — none did, but the migration checks rather than assumes. Every
name-bearing object this migration could find was renamed alongside the table (indexes, the unique
constraint, both draft_kind/shape CHECKs, both triggers and their guard function, the RLS policy, and —
caught only by actually regenerating `database.types.ts` against it — six foreign key constraints
`alter table rename` does not touch), plus both registry rows (`product_module_resources`,
`audit_entity_manifest`) that key on the name as data rather than resolving by OID. Verified against a
real local Postgres instance, not just read: full pgTAP suite green (3050 tests) including new Tier 4
coverage for the vitals lane, which shipped with none; `db lint`/`db advisors --fail-on error` clean;
regenerated types match the schema with zero remaining references to either retired name.
