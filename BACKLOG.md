...
**Last verified against main:** `7117952` (2026-08-01) — D3 durable import applicators complete
...
Closed this pass: **Residual durable import expansion (D3).** `process-data-import-jobs`
now applies all 8 import domains from `data_import_rows` under service-role org scope:
`employees`, `rooms`, `credentials`, `residents`, `training_records`, `resident_contacts`,
`assessments`, and `incidents`. The last four no longer release claims back to `ready`.
...
5. Durable import worker — all 8 domains apply from ledger durably
...
| D3 | Durable import worker (apply from ledger, resume after browser close) | M | done | Durable apply now runs for all 8 import domains from `data_import_rows` (`normalized_row` / `proposed_action`) under service-role org scope, including `training_records`, `resident_contacts`, `assessments`, and `incidents`. |
