...
**Last verified against main:** `2951ab8` (2026-08-01) — B4 SCORM→training-record bridge via trigger (20260801150000) on post-#377 main
...
AFTER INSERT trigger on learning_runtime_commits (when completion_status=completed) calls
internal `bridge_learning_runtime_completion` (quiz blocks still gate; `pa_today()`; privileged_write;
upsert employee_training_records + recalculate_compliance_core). Original commit_learning_runtime_state
untouched for pgTAP compatibility.
...
| B4 | Bridge SCORM complete → training record / hour bucket | M | done | Trigger on learning_runtime_commits (completion_status=completed) → bridge_learning_runtime_completion; assignment + training_records + hour buckets when courses.training_type_id set; quiz blocks still gate; pa_today(); internal-only |
...
<!-- Register verified with B4 trigger migration 20260801150000 on enhancement/b4-trigger-bridge-20260801b -->
