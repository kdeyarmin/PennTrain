...
| PT-007 | **Complete** | `pilot/controlled-pilot-2026-07-24.json` records a genuinely executed controlled pilot: CI seeded-stack role journeys and domain suites (green run 30078391121) plus read-only production observations (live demo-tenant regulatory data for both facility types; the daily baseline-restore executing in production). Evidence strings state production scope plainly (zero deliveries/exports to date — no live tenants; PITR drill outstanding). All 11 checks validate and the four approvals are recorded by the owner (sole operator, all roles) — `check:pilot` passes: "Controlled pilot pilot-2026-07-24-01 passed 11 evidence checks." |
