...
| PT-007 | **Evidence recorded — approvals pending** | `pilot/controlled-pilot-2026-07-24.json` records a genuinely executed controlled pilot: CI seeded-stack role journeys and domain suites (green run 30078391121) plus read-only production observations (live demo-tenant regulatory data for both facility types; the daily baseline-restore executing in production). Evidence strings state production scope plainly (zero deliveries/exports to date — no live tenants; PITR drill outstanding). All 11 checks validate; `check:pilot` goes green once the four human approvals are typed in. |
