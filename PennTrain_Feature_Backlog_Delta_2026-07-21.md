...
| **E2** — Server-side pagination adoption (+ E16 per page) | In progress. `PolicyDocuments` converted to `usePaginatedDomainList` with URL-persisted search + page (E16 folded in); `policy_documents` added to the shared config. Remaining unbounded pages fall into two groups that each need more than a hook swap: **summary-card pages** — `Complaints`, `ConfidentialIncidents`, `EvidenceRoom`, `WorkQueue` — compute whole-dataset metric tiles (`all.filter(...).length`) that server-side paging breaks, so each needs a `get_*_list_summary` RPC (a migration) alongside the paged rows; **joined pages** — `Documents` (employee join + employee/type filters), `Practicums` (employee join) — need the join preserved (fetch related rows separately or paginate their own domain hook). Do the summary-page RPCs and the joined-page conversions as follow-ups. | P1, M |
