...
| **E2** — Server-side pagination adoption (+ E16 per page) | **Done (7 of 7).** `PolicyDocuments` and `ConfidentialIncidents` use the shared `usePaginatedDomainList` config (no joins). `Complaints`, `EvidenceRoom`, and `WorkQueue` use the **summary-card** pattern with a dedicated join-preserving hook. Each summary-card page adds a SECURITY INVOKER `get_*_list_summary` RPC, pgTAP-tested against direct RLS-scoped counts: `get_complaint_list_summary` (`20260721180000`), `get_evidence_collection_list_summary` (`20260721190000`), `get_confidential_intake_list_summary` (`20260721200000`), and `get_work_item_list_summary` + `get_work_item_queue` (`20260721210000`). `Documents` uses a dedicated `usePaginatedDocuments` hook that keeps the uploader-employee join plus the facility/employee/type filters; `Practicums` uses a dedicated `usePaginatedPracticums` hook (it resolves employee names client-side from a separate map rather than a Supabase join, so no embed is preserved there). `WorkQueue` was the hard case: its overdue-first → priority → due_at sort depends on "now" and an enum rank, so `get_work_item_queue` computes the sort, filters, and one page in SQL and returns `{count, rows}` as jsonb with the facility/owner/template embeds intact. All fold in E16 (URL-persisted filters + page). | P1, M — **shipped** |
...
- **Slice 2** — help/account IA de-duplicated. `What's new` and `Announcements` were exposed in all three surfaces (header `?` dropdown, header avatar menu, sidebar footer). They now have a single canonical home in the `?` help/updates dropdown (which already carries the changelog unread badge); the two account menus are trimmed to account actions only — Account security, Notification settings, Log out — so each destination has one home. `Help center` stays in the `?` menu and the sidebar Help nav.

All three PT-045 slices are shipped. The only remaining nicety is a full entity-aware breadcrumb *hierarchy* (section → entity); the entity itself already shows in the page title and browser tab.
