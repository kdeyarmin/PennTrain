...
| **E2** — Server-side pagination adoption (+ E16 per page) | Status: adopted on exactly 5 pages (`Alerts`, `Incidents`, `InspectionItems`, `Residents`, `Violations` — the only `usePaginatedDomainLists` importers under `src/pages`). Verified-unbounded remainder (zero `usePaginated`/`.range(` references): `Complaints.tsx`, `ConfidentialIncidents.tsx`, `Documents.tsx`, `PolicyDocuments.tsx`, `Practicums.tsx`, `WorkQueue.tsx`, `EvidenceRoom.tsx`. Fold E16 URL-filter persistence into each page as it converts. | P1, M |
...
| **E10** — Notification spend caps | Confirmed open and still ordered before any external fan-out expansion. | P1, S–M |
