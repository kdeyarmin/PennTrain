...
| **E2** — Server-side pagination adoption (+ E16 per page) | In progress (2 of 7 done). `PolicyDocuments` converted via the shared `usePaginatedDomainList` config. `Complaints` converted as the reference **summary-card** conversion: a `get_complaint_list_summary` RPC (`20260721180000`, SECURITY INVOKER, pgTAP-tested) backs the metric tiles over the whole filtered scope, and a dedicated `usePaginatedComplaints` hook keeps the facility/resident/investigator/incident joins while paging server-side. Both pages fold in E16 (URL-persisted filters + page). Remaining: **summary-card pages** `ConfidentialIncidents`, `EvidenceRoom`, `WorkQueue` (each needs its own `get_*_list_summary` RPC, mirroring the Complaints pattern); **joined pages** `Documents`, `Practicums` (paginate their own domain hook to preserve the employee join). | P1, M |
