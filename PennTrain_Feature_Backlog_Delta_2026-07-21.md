...
| **E2** — Server-side pagination adoption (+ E16 per page) | In progress (4 of 7 done). `PolicyDocuments` and `ConfidentialIncidents` use the shared `usePaginatedDomainList` config (no joins). `Complaints` and `EvidenceRoom` use the **summary-card** pattern with a dedicated join-preserving hook. Each converted summary-card page adds a SECURITY INVOKER `get_*_list_summary` RPC, pgTAP-tested against direct RLS-scoped counts: `get_complaint_list_summary` (`20260721180000`), `get_evidence_collection_list_summary` (`20260721190000`), `get_confidential_intake_list_summary` (`20260721200000`). All fold in E16 (URL-persisted filters + page). Remaining: **summary-card page** `WorkQueue` (needs its own `get_*_list_summary` RPC); **joined pages** `Documents`, `Practicums` (paginate their own domain hook to preserve the employee join). | P1, M |
