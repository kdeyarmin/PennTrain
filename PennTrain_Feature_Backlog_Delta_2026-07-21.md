...
| **E2** — Server-side pagination adoption (+ E16 per page) | In progress (3 of 7 done). `PolicyDocuments` via the shared `usePaginatedDomainList` config. `Complaints` and `EvidenceRoom` converted with the **summary-card** pattern: a `get_*_list_summary` RPC (`get_complaint_list_summary` `20260721180000`, `get_evidence_collection_list_summary` `20260721190000` — both SECURITY INVOKER, pgTAP-tested against direct RLS-scoped counts) backs the metric tiles over the whole filtered scope, and a dedicated paginated hook keeps each list's joins (complaint facility/resident/investigator/incident; collection facility name). All fold in E16 (URL-persisted filters + page). Remaining: **summary-card pages** `ConfidentialIncidents`, `WorkQueue` (each needs its own `get_*_list_summary` RPC); **joined pages** `Documents`, `Practicums` (paginate their own domain hook to preserve the employee join). | P1, M |
